...
**Status:** **v0.7.2 — implementation-ready; discovery,
journal-bundle, and `parity_map` contract polish.** This release
keeps v0.7.1's contracts intact and adds a small implementer-facing
clarity pass: finite ordered bootstrap block-size candidates,
single-pass `parity_map` digest sizing guidance, journal-less
forward-validation seed rules, and stale `ObjectCommitBundle` naming
cleanup. No on-tape parity-engine change.

**Status (v0.6):** persistence decoupled from any RDBMS. Remanence
(including Layer 5) runs end-to-end on local files with **no
database process**. 3c persists committed map entries through a
`TapeFileJournal` trait (§10); the default `FileTapeFileJournal` is a
single append-only local file (`std` + CBOR + CRC-64). The durable,
portable source of truth is the **tape itself** (bootstraps +
`parity_map`/directory, §5.6); the journal is the local resume/commit
record; an external *orchestrator* (separate software, out of
process) MAY keep a real RDBMS but Remanence neither hosts nor
requires it. The journal's fsync is a commit point with the same
status as the synchronous filemark and is pinned to the trusted local
volume (§7.7, §10.6). Layer 5's optional local query catalog is
embedded SQLite — a rebuildable, non-authoritative mirror, never a
server (§10.7).

v0.5 folded the v0.2 implementation addendum: sidecar metadata is
replicated (primary + tail + footer, §5.5); catalog-less map
reconstruction is robust to a damaged sidecar header via a bootstrap
sidecar directory and an external `parity_map` control file (§5.6,
§8.1); volatile sidecar deferral is prohibited and replaced by the
**object commit bundle** with a bounded one-epoch restart invariant
(§7.4, §7.8); bulk/epoch recovery with a memory cap is specified
(§9.2–§9.3); drive hardware compression is a verified hard-false
precondition (§6.1.1, §11.6, §12); the GF(2⁸) codec is owned in-tree
(§13); and the adoption scope is stated explicitly (§1). The parity
architecture is unchanged and settled. Mock/in-memory and codec work
(RS, sidecar, bootstrap, filemark-map, recovery) is ready to build.
...
(1) Spool thresholds, `BootstrapPlacementPolicy` cadence, and
...
correctness gate (impl step 11.6). (3) The open-epoch rebuild
re-read (§7.8 step 8), the commit durability barrier (§7.7), and the
`TapeFileJournal` fsync / torn-trailing-record replay (§10) want
validation on real hardware including a deliberate power-loss cycle,
on the deployed local filesystem (akash ZFS) (impl steps
11.18a/b/c, 11.19). (4) The exact MODE SELECT / MODE SENSE page used
to disable and verify drive compression must be proven on the
deployed LTO-9 models (§6.1.1, §11.6). (5) Upstream object splitting
(§7.4) is the mitigation for the large-object burst-rewrite cost;
whether it is a firm orchestrator policy or an advisory `SHOULD`
depends on whether the workload contains indivisible multi-TB single
objects.

*Items closed in v0.6:* Remanence no longer depends on any RDBMS;
map persistence is the `TapeFileJournal` trait with a local
append-only default, and the old "real-Postgres deferrable-constraint
prototype" item is dropped (Postgres, if present, belongs to the
external orchestrator). *Items closed in v0.5 (were "accepted v1
simplifications" in v0.4.4):* sidecar metadata is now replicated and
the "damaged-sidecar-header degrades the whole scanned map" behavior
is eliminated (§5.5, §5.6, §8.1).

**Changes from v0.7.1 (v0.7.2 — implementer-polish patch).** No
on-tape format or parity-engine change. This pass incorporates the
post-v0.7.1 review's clarity edits:

- **`parity_map` digest sizing is effectively single-pass (§5.6.1).**
  The provisional payload reserves a full 32-byte
  `canonical_map_digest`, so inserting the real digest cannot change
  CBOR length, `M`, or `block_count`; the repeat check remains only a
  defensive assertion.
- **Bootstrap block-size fallback is finite and ordered (§8.1,
  §15.1).** Implementations try the configured/catalogued size first,
  then a bounded default candidate list (`256 KiB`, `512 KiB`, `1 MiB`)
  unless an operator supplies an explicit finite list. No open-ended
  block-size brute force.
- **Journal-less suffix validation seeds `current_epoch_fill` explicitly
  (§7.8).** The suffix walk initializes the live partial-epoch fill from
  the attested bootstrap prefix: `T - W`, where `T` is
  `directory_scope_total_data_ordinals` and `W` is
  `directory_scope_highest_protected_ordinal`.
- **Stale `ObjectCommitBundle` type references removed.** The active
  type is `CommittedBundleKind::Object`; “object commit bundle” remains
  as prose only.

**Changes from v0.7 (v0.7.1 — implementation-contract cleanup).** No
parity-engine change; only discovery, journal, and control-file
contracts are tightened.

- **Bootstrap discovery is BOT → hints → full filemark scan (§8.1,
  §15.1).** Non-BOT bootstraps are content-driven by
  `BootstrapPlacementPolicy`, not fixed fractional positions. Any
  journal/MAM/catalog/operator positions are accelerators only; a
  catalog-less reader falls back to a full filemark scan and chooses
  the highest-scope valid bootstrap.
- **Raw block-size configuration is explicit (§4.5, §8.1).**
  `RawTapeSource::configure_fixed_block_size` is part of the raw
  interface because unknown-tape bootstrap discovery must reconfigure
  the drive before each candidate-size read.
- **Generic journal bundles (§6.1, §10.2).** `CommittedBundle` is no
  longer object-only. `Object`, `Control`, `ResumeSidecars`, and
  `Finish` bundles all use the same synchronous-filemark → journal
  fsync commit discipline; `checkpoint()` and standalone
  `write_bootstrap()` produce `Control` bundles.
- **Strict `parity_map` wire layout and digest order (§5.6.1).** The
  `parity_map` header/footer now has byte offsets, CRC ranges, and
  block-packing rules. When a bootstrap references an external
  `parity_map`, the writer allocates both structural entries before
  computing the canonical map digest, avoiding self-reference.
- **Tape-only resume forward validation (§7.8).** A journal-less
  writer reconstructs committed bundles from the last on-tape
  bootstrap by requiring each accepted object to be followed by the
  sidecars it completed; otherwise the suffix is abandoned before the
  object, preserving `T − W < one epoch`.
- **Journal scheme copy clarified (§10.1, §10.3).** The bootstrap is
  the authoritative scheme source; the journal repeats the scheme only
  as a consistency check and is rejected on mismatch.
- **Normative wording now uses journal/committed-bundle terminology.**
  External catalog / SQLite state is a downstream query mirror, not a
  3c commit boundary.

**Changes from v0.6 (v0.7 — checkpoint / session lifecycle).** No
on-tape format or parity-engine change.

- **`checkpoint()` write-session call (§6.1).** Distinct from
  `finish()`. Flushes pending state, writes a fresh non-final
  (`is_final_map = false`) prefix bootstrap, commits the in-flight
  bundle through the journal, and leaves the tape resumable.
  `finish()` remains the terminal close (final bootstrap, tape closed
  forever).
- **Clean vs unclean session end defined in 3c terms (§7.8).** A
  clean end is a returned `checkpoint()` or `finish()`; anything else
  (crash, power loss, cartridge pulled mid-session) is unclean and
  takes the existing crash/tape-only resume path. 3c has no notion of
  a physical mount/unmount; that is a Layer 3a/Layer 5 concern.
- **Layer 5 obligation: checkpoint before `UNLOAD` (§13.1).** Layer 5
  MUST call `checkpoint()` and see it return before issuing a SCSI
  `UNLOAD` on a tape it intends to resume; otherwise the tape is in
  the unclean-end state and pays the slower tape-only-resume scan on
  remount (a cost, not data loss).
- **Content-driven `BootstrapPlacementPolicy` (§7.3).** Replaces the
  fixed "BOT / ~1/3 / ~2/3 / near-EOD" schedule with a staleness floor
  (a bootstrap every `bundles_per_bootstrap` committed bundles or
  `ordinals_per_bootstrap` newly-protected ordinals, whichever first),
  an end-of-medium taper that tightens the floor in the last ~10% and
  ~1% of estimated capacity, and a `min_physical_separation_blocks`
  so the taper cannot collapse late copies into a single damage band.
  A `checkpoint()` always writes a bootstrap and resets the staleness
  counters. BOT and final bootstraps remain mandatory.

**Changes from v0.5 (v0.6 — map-persistence decoupling).** No
architectural change to the on-tape format or the parity engine.
Removes the implicit external-RDBMS dependency so Remanence runs on
local files alone.

- **`TapeFileJournal` trait + `FileTapeFileJournal` default (§10).**
  3c persists committed map entries through a trait, not a database.
  The default is one append-only file per tape (`std` + CBOR +
  CRC-64), committing an atomic *bundle* per object (§7.4) with an
  fsync that is the commit point. Replay drops a torn trailing
  record, matching §7.8.
- **Tape is authoritative; journal is the resume/commit record;
  external store is optional (§10, §10.5).** `catalog_tape_files` is
  reframed as one possible backing an external catalog may provide,
  not a 3c requirement and not required to be Postgres.
- **Journal durability precondition (§7.7, §10.6).** The journal is
  pinned to the same trusted local volume as the parity spool; a
  network filesystem, tmpfs, or untrusted-flush device is rejected at
  session open, mirroring the §6.1.1 compression precondition.
- **Layer 5 local catalog = embedded SQLite (§10.7).** A rebuildable,
  non-authoritative cross-tape query mirror — no server. A real RDBMS,
  if used, is the external orchestrator's and is out of process.
- **No-database build gate (§14).** The full
  write→unmount→remount→append→read→recover cycle must pass with no
  database process running (journal-only), on mock and scratch LTO.
...
current local journal / on-tape map model replaces the earlier
"catalog-authoritative" wording.
...
- **No volatile sidecar deferral; object commit bundles (§7.4, §7.8).**
  ...
  one atomic `CommittedBundleKind::Object` bundle. The hard invariant
...
- **Drive compression hard-false precondition (§6.5, §11.6, §12).**
  ...
  any parity-protected write, recorded in the bootstrap and journal/query
  mirror; a tape recording `drive_compression=true` is refused for 3c recovery.
...
- **Scope statement (§1) and on-tape format docs (§10.8).** 3c
...
- **Resume-emitted sidecars commit like ordinary sidecars (§7.8).**
  ...
  data. Those sidecars now have explicit journal bundle
...
- **Commit durability barrier (§7.7).** `RawTapeSink::write_
  ...
  the journal-commit ordering is fixed: blocks → synchronous
  filemark → capture position → commit journal bundle. Mirroring to
  SQLite/RDBMS before the durable filemark and journal commit is forbidden.
- **Restart / append-after-crash (§7.8).** The append point is the
  trailing filemark of the last *journal-committed* tape file
...
- **Commit wording aligned with the parity-lag model (§7.5.1).**
  ...
  *data-committed* once its archive + filemark are durable and
  the journal records the committed bundle and any query mirror records its accurate `parity_state`; full
...
- **Provisional map-entry semantics (§7.5.2).** Sidecar/bootstrap
  ...
  bootstrap or journal commit is made, and recovery falls back to
...
- **`write_bootstrap` computes its own digest (§4.4, §6.1,
  §7.3.1).** Removed caller-supplied `filemark_map_digest` and later
  the vestigial sequence/final-map parameters entirely; the current
  API is sink-owned `write_bootstrap()` for non-final control points
  and `finish()` for the final bootstrap. A caller-supplied digest
  could not include the bootstrap's own map entry, the exact thing
  §7.3.1 requires.
...
- **Parity epochs span objects.** RS neighborhoods ("epochs")
  ...
  §5.4)
- **Filemark map** is the new structural element, committed locally
  through the `TapeFileJournal`, mirrored onto tape through bootstraps
  plus directory/`parity_map`, and optionally mirrored into a query
  catalog. (§5.6, §8.1, §10)
...
- **Bootstrap not parity-protected** (it defines the scheme, so
  parity can't recover it); replicated by BOT + content-driven/checkpoint placement,
  CRC/hash-validated, with a filemark-map digest. (§5.6, §8.1)
- **The map has a durable commit record.** v0.2's "no new map fields"
  is superseded by the journal-backed `TapeFileEntry` model (§10), with
  optional external catalog mirrors.
...
- Bootstrap blocks written as standalone tape files and found by
  magic/CRC scan. *(Retained as replicated bootstrap tape files;
  placement is now content-driven in v0.7 rather than fixed-fractional.)*
...
for object in objects {
    ...
    // ... record/commit (tfn, close) through the journal ...

    // Writer policy decides when to emit further bootstrap copies
    // at content-driven object-boundary checkpoints (§7.3).
}
...
*consumes* `RawTapeSink` / `RawTapeSource` (the physical tape surface
adapted from `DriveHandle`) and *provides* the body-facing
`BlockSink` / `BlockSource` surface consumed by formats. This is
intentional — raw tape operations, filemarks, bootstraps, sidecars,
and block-size reconfiguration stay below 3c, while body formats see
only object-local `BodyLba` / `BodyPosition`.
...
Note: v0.2's `catalog_hint_lbas` is removed. The authoritative
filemark map is on tape (bootstraps + directory/`parity_map`) and in
the local `TapeFileJournal` resume record (§10); optional catalogs may
mirror it. Catalog-less readers reconstruct-and-validate via the
...
pub trait RawTapeSource {
    /// Configure the drive's fixed block size for subsequent reads.
    /// Normal mounts call this once with the configured/catalogued
    /// block size. Unknown-tape bootstrap discovery (§8.1) calls it
    /// for each candidate size before attempting a candidate read.
    /// The raw adapter MUST issue the actual Layer 3a drive
    /// configuration, not merely resize a userspace buffer.
    fn configure_fixed_block_size(&mut self, block_size: u32)
        -> Result<(), FormatError>;
...
pub trait RawTapeSink {
    ...
    /// variant), never IMMED=1 / MTWEOFI. 3c commits a journal bundle
    /// only AFTER this returns; any query mirror is updated downstream.
    /// Returns position + EW/EOM.
...
- **Bootstrap tape files** — the root of trust, replicated at
  BOT, checkpoint/final positions, and content-driven placement-policy
  positions (§5.6, §7.3).
...
footer's; its scheme parameters match the bootstrap scheme (and any
journal consistency copy); and its protected ordinal range is sane and
non-empty.
...
- The writer places **multiple bootstrap copies** as independent
  filemark-delimited tape files. BOT is mandatory; later copies are
  placed by the content-driven `BootstrapPlacementPolicy` (§7.3) and
  by explicit `checkpoint()` calls. Each copy is self-validating by
  CRC/hash.
- A reader tries BOT first, then any available position hints
  (journal/MAM/external catalog/operator hints), and finally a full
  filemark scan (§8.1). Hints accelerate discovery but are never
  required for catalog-less recovery. Losing some copies to damage is
  fine as long as one validating copy remains.
...
This keeps bootstrap discovery outside the ordinal stream it
logically precedes. The old v0.2 phrase "magic-scan at expected
positions" is now only a fast path: BOT and optional hints are tried
first, and a full filemark scan is the catalog-less fallback.
...
happens to contain `REM\x00BOO\x01` at the start of a probed
tape file — is mitigated by requiring a complete bootstrap validation:
header CRC, payload CRC, schema, block size, tape UUID, sequence, and
digest fields must all parse and agree. Magic alone never classifies a
bootstrap.
...
It is persisted in the local **TapeFileJournal** (§10) for resume
and carried structurally on tape through bootstraps plus the
sidecar directory / `parity_map`. Optional catalogs may mirror it,
but are not the 3c authority. A **digest** of the map projection is
carried in each bootstrap so a catalog-less reader can reconstruct
the map by scanning (§8.1) and validate the reconstruction. The map
is what `ParitySource` uses to translate `(tape_file_number,
...
sidecar files, and — like sidecars — internally replicated. The
layout below is normative. All multi-byte integers are little-endian;
all CRCs are CRC-64/XZ (§5.5).

```
parity_map tape file:
  primary copy: blocks 0 .. M-1
  tail copy:    blocks M .. 2M-1
  footer:       block 2M
  filemark

block0_payload_capacity = block_size - 8 - 0xB0
continuation_payload_capacity = block_size - 8
if payload_len <= block0_payload_capacity:
    M = 1
else:
    M = 1 + ceil((payload_len - block0_payload_capacity)
                 / continuation_payload_capacity)

The writer computes M from the encoded canonical-CBOR payload.
Unused bytes in every copy block are zero-filled and covered by that
block's CRC.
```

**ParityMapPayload (canonical CBOR).** The payload is byte-identical
in the primary and tail copies:

```cbor
ParityMapPayload = {
    1: tstr,             ; format_id = "rem-parity-map-v1"
    2: bytes .size 16,   ; tape_uuid
    3: uint,             ; sequence, sink-owned
...
**ParityMapHeader, copy block 0.** The header appears at the start
of each copy. Payload bytes begin at offset `0xB0` and continue across
that copy's blocks. If the payload spans multiple blocks, continuation
blocks contain only payload bytes + zero-fill + a trailing block CRC.

```
ParityMapHeader (copy block 0; LITTLE-ENDIAN):
  0x00  u8[8]    magic = HMAC-SHA256(tape_uuid, b"REM\x00PMAP\x01")[0..8]
  0x08  u16le    parity_map_version (= 1)
  0x0A  u8       copy_kind          (0 = primary, 1 = tail)
  0x0B  u8       reserved           (zero)
  0x0C  u32le    header_len         (= 0xB0)
  0x10  u8[16]   tape_uuid
  0x20  u32le    sequence
  0x24  u32le    block_size
  0x28  u64le    payload_len
  0x30  u64le    copy_block_count   (= M)
  0x38  u64le    primary_start_block (= 0)
  0x40  u64le    tail_start_block    (= M)
  0x48  u64le    footer_block_index  (= 2M)
  0x50  u32le    directory_scope_tape_file_count
  0x54  u8       is_final_directory  (0 or 1)
  0x55  u8[3]    reserved            (zero)
  0x58  u64le    directory_scope_total_data_ordinals
  0x60  u64le    directory_scope_highest_protected_ordinal
  0x68  u8[32]   canonical_map_digest
  0x88  u8[32]   payload_sha256      (SHA-256 over payload bytes only)
  0xA8  u64le    header_crc64        (covers 0x00..0xA8, exclusive)
  0xB0  ...      payload bytes begin here
  ...            zero-fill to block_size - 8
  block_size-8   u64le block_crc64   (covers 0x00..block_size-8)
```

**ParityMap continuation blocks.** For copy blocks `1..M-1`, bytes
`0..block_size-8` carry the next payload bytes followed by zero-fill;
the final 8 bytes are `block_crc64` over bytes `0..block_size-8`.
The reader verifies every block CRC, concatenates only the first
`payload_len` payload bytes, checks `payload_sha256`, then parses
canonical CBOR.

**ParityMapFooter (block `2M`).** The footer lets a scanner locate
the tail copy even when the primary header is unreadable:

```
ParityMapFooter (one block; LITTLE-ENDIAN):
  0x00  u8[8]    magic = HMAC-SHA256(tape_uuid, b"REM\x00PMAPFOOT\x01")[0..8]
  0x08  u16le    footer_version (= 1)
  0x0A  u16le    reserved       (zero)
  0x0C  u32le    block_size
  0x10  u8[16]   tape_uuid
  0x20  u32le    sequence
  0x24  u32le    directory_scope_tape_file_count
  0x28  u64le    directory_scope_total_data_ordinals
  0x30  u64le    directory_scope_highest_protected_ordinal
  0x38  u64le    primary_start_block (= 0)
  0x40  u64le    tail_start_block    (= M)
  0x48  u64le    copy_block_count    (= M)
  0x50  u64le    footer_block_index  (= 2M)
  0x58  u8[32]   canonical_map_digest
  0x78  u8[32]   payload_sha256
  0x98  u64le    footer_crc64        (covers 0x00..0x98, exclusive)
  ...            zero-fill to block_size - 8
  block_size-8   u64le footer_block_crc64 (covers 0x00..block_size-8)
```

A `parity_map` copy is usable only if: magic matches, header CRC and
all block CRCs pass, `payload_sha256` matches the payload, the CBOR
payload is canonical and structurally valid, and the payload's
`canonical_map_digest` / directory scope fields match the header and
footer. If both primary and tail are valid, their payload SHA-256 and
canonical map digest MUST match.
...
**Commit ordering when an external map is needed (normative).** A
`parity_map` referenced by a bootstrap must be part of the same
committed prefix as that bootstrap. The digest includes the structural
entries of **both** control tape files, but never their payload bytes.

```
1. Build the SidecarEpochDirectory over the intended committed map scope.
2. Encode a provisional ParityMapPayload and compute M / block_count.
3. Assign the parity_map tape_file_number and append its provisional
   structural map entry: kind=parity_map, block_count=2M+1.
4. Assign the bootstrap tape_file_number and append its provisional
   structural map entry: kind=bootstrap, block_count=1.
5. Compute canonical_map_digest over the canonical map projection that
   includes both provisional structural entries.
6. Encode the final ParityMapPayload with that digest; compute
   payload_sha256 and verify M / block_count did not change. In the
   normal canonical encoding this check is a defensive assertion, not
   a real fixed-point loop: step 2's provisional payload MUST reserve
   the full `bytes .size 32` `canonical_map_digest` field, so replacing
   the placeholder with the real 32-byte digest cannot change
   `payload_len`, M, or `block_count`. If an implementation still
   detects a change, repeat from step 3 with the corrected block_count
   and emit an implementation-warning audit event.
7. Write the parity_map tape file (primary, tail, footer) and its
   synchronous filemark.
8. Encode the bootstrap with a ParityMapReference to that parity_map,
   carrying payload_sha256 and canonical_map_digest.
9. Write the bootstrap tape file and its synchronous filemark.
10. Commit parity_map + bootstrap entries in one journal Control or
    Finish bundle (§10.2). Until `journal.commit_bundle()` returns,
    both entries remain provisional (§7.5.2).
```

This ordering is the non-circularity rule for external maps. The
canonical map digest includes `(tape_file_number, kind, block_count)`
for the `parity_map` and bootstrap, but excludes their content hashes,
bootstrap payload bytes, and parity_map payload bytes.
...
pub struct ParitySink<'a> {
    ...
    journal: &'a mut dyn TapeFileJournal,  // durable commit record (§10.2); the commit point
    scheme: ParityScheme,
    tape_uuid: [u8; 16],
    ...
    filemark_map: FilemarkMapBuilder,      // in-RAM cache; durable copy is the journal + tape
    next_bootstrap_sequence: u32,          // sink-owned (review #8); 0,1,2,...
    audit_hook: Option<Arc<dyn ParityAuditHook>>,
}

impl<'a> ParitySink<'a> {
    /// `journal` is the durable commit record (§10.2). Its volume is
    /// validated against the §10.6 precondition at construction; an
    /// untrusted-flush volume returns SessionOpen/JournalError before
    /// any tape write. The catalog/orchestrator (if any) is NOT passed
    /// here — 3c depends only on the journal trait.
    pub fn new(
    ...
    ///
    /// Commit semantics: when called as a standalone operation between
    /// objects, this writes and commits a `CommittedBundleKind::Control`
    /// bundle. When called internally while closing an object, during a
    /// sidecar burst, or during `finish()`, the bootstrap entry is folded
    /// into the already-open Object/Finish bundle (§10.2). In all cases
    /// the bootstrap tape file and its synchronous filemark must complete
    /// before the containing bundle can be committed.
    pub fn write_bootstrap(&mut self)
        -> Result<u32, ParityError>;       // returns bootstrap tape_file_number

    /// Resumable clean close (NOT terminal). Use before setting a tape
    /// down to resume later (e.g. Layer 5 calls this before SCSI
    /// UNLOAD; §13.1). checkpoint():
    ///   1. asserts no object is open (call between objects, like
    ///      write_bootstrap); a checkpoint mid-object is an Invariant
    ///      error — close or finish the object first.
    ///   2. does NOT close the current partial epoch (that is finish()
    ///      only, §5.4); the open epoch stays open and is rebuilt on
    ///      resume (§7.8). The committed prefix therefore still
    ///      satisfies T − W < one epoch.
    ///   3. writes a fresh NON-FINAL prefix bootstrap (is_final_map =
    ///      false, sink-assigned sequence; §7.3.1) attesting the
    ///      committed prefix, and commits it as a
    ///      `CommittedBundleKind::Control` bundle through the journal
    ///      (§7.7, §10.2) so tape and journal agree on the prefix.
    ///   4. resets the BootstrapPlacementPolicy staleness counters
    ///      (§7.3).
    /// After a returned checkpoint() the tape is in the CLEAN
    /// session-end state (§7.8): a later session reopens and resumes
    /// from this bootstrap's attested prefix, journal-fast or tape-only.
    /// The session handle remains valid; appending may continue, or the
    /// caller may drop the handle / Layer 5 may UNLOAD.
    ...
    /// sequence and stamps is_final_map=true; §7.3.1), commit a
    /// `CommittedBundleKind::Finish` bundle, and return the complete
    /// filemark map for downstream query/catalog mirrors. Sequence
    /// numbering for all bootstraps — intermediate and final — is
    /// sink-owned, so there is no caller parameter (review #8).
    /// Terminal: the tape is closed forever (contrast checkpoint()).
    pub fn finish(self) -> Result<TapeGeometry, ParityError>;
}

pub struct CheckpointResult {
    /// The non-final bootstrap written by this checkpoint.
    pub bootstrap_tape_file_number: u32,
    /// Committed prefix attested by it.
    pub tape_file_count: u32,
    pub highest_protected_ordinal: u64,    // W
    pub total_committed_ordinals: u64,     // T
}
...
pub struct ObjectCloseResult {
    ...
    /// TapeFileEntry data. May be empty (small objects).
    pub sidecars_emitted: Vec<SidecarTapeFile>,
    /// The tape's protection watermark AFTER these sidecars. The
    /// journal bundle carries this value; any SQLite/external query
    /// mirror recomputes object parity_state downstream (§10.1).
    /// Derivable from sidecars_emitted, but surfaced explicitly to
    /// avoid transaction bugs.
    pub highest_protected_ordinal: u64,
}
...
The bootstrap and journal/query mirror record `drive_compression = false` and
...
the scheme) and obtained the filemark map (from the journal/trusted
mirror, or scan-reconstructed and digest-validated — §8.1).
...
/// Needed for catalog-less recovery; skippable when a trusted journal
/// or external mirror supplies the map.
...
/// Preferred: from TapeFileJournal or a trusted downstream mirror
/// (Complete). Fallback (journal-less/catalog-less): scan-reconstruct
/// and validate against the AUTHORITATIVE bootstrap's digest — fully
/// if is_final_map, else only the attested prefix (MapScope::Prefix).
pub fn acquire_filemark_map(
    raw: &mut dyn RawTapeSource,
    authoritative_bootstrap: &BootstrapPayload,
    trusted_map: Option<FilemarkMap>,
) -> Result<ScopedFilemarkMap, ParityError>;
```

Step 1's bootstrap gives the scheme and `tape_uuid`; step 2's
gives the digest that validates the map. If the journal or a trusted
external mirror already supplies a complete map, step 2 can be skipped.
If not, `acquire_filemark_map` scan-reconstructs by walking the tape
file by file, classifying each by magic/CRC (not magic alone), then
checks it against the authoritative digest. Only after both the scheme
and a validated map are in hand is a `ParitySource` constructed.
...
for object in objects_to_write {
    ...
    let close = parity_sink.finish_object()?;   // filemark + completed-epoch sidecars
    layer5.record(tape_file_number, close);      // optional query/catalog mirror
...
and the journal/query mirror MUST reflect that rather than marking them
protected.
...
ObjectCommitRecorded
    the journal has committed the bundle containing this object and
    any sidecars/control files generated with it; query mirrors, if
    present, reflect the correct parity state downstream.
```
...
The query layer records, per object: `first_parity_data_ordinal`,
...
still applies), provided the journal/query mirror does not pretend the
uncovered ordinals are protected.

### 7.3 Bootstrap placement policy

A bootstrap's recovery value is not its physical position but **how
much committed map it attests that no later surviving copy already
covers** — i.e. how much of the table of contents you keep if the
*tail* of the tape is lost. That quantity is best measured in
committed map *content* (bundles / newly-protected ordinals), not in
linear tape distance: a region of many small objects accrues map fast,
a single multi-TB master accrues almost none for a long stretch. v0.7
therefore replaces the old fixed "BOT / ~1/3 / ~2/3 / near-EOD"
schedule with a content-driven policy.

```rust
pub struct BootstrapPlacementPolicy {
    /// Staleness floor: emit a bootstrap once this many committed
    /// bundles have accrued since the last bootstrap. (akash-tunable)
    pub bundles_per_bootstrap: u32,
    /// Staleness floor: ...or this many newly-protected ordinals,
    /// whichever trips first.
    pub ordinals_per_bootstrap: u64,
    /// End-of-medium taper: divisors applied to the two floors as
    /// estimated remaining capacity (via the §7.5 reserve) crosses
    /// each threshold. e.g. [(0.10, 2), (0.01, 4)] = halve the floor
    /// in the last 10%, quarter it in the last 1%.
    pub eom_taper: Vec<(f64 /*remaining_fraction*/, u32 /*divisor*/)>,
    /// Dispersal guard: never place two bootstraps closer than this
    /// many physical blocks apart, so the taper cannot collapse late
    /// copies into a single contiguous-damage band's width. A trip of
    /// a staleness floor while inside this distance is deferred to the
    /// next object boundary past it.
    pub min_physical_separation_blocks: u64,
}
```

Placement rules (the sink owns assignment; thresholds are
akash-specific tuning constants):

- A bootstrap is **mandatory at BOT** and as the **final** bootstrap
  at `finish()` (the latter carries `is_final_map = true` over the
  complete map).
- Between those, emit a non-final bootstrap when a staleness floor
  trips (`bundles_since_last >= bundles_per_bootstrap` OR
  `protected_ordinals_since_last >= ordinals_per_bootstrap`), at the
  next **object boundary** (never mid-object), subject to
  `min_physical_separation_blocks`.
- Apply the `eom_taper` divisors to the floors as remaining capacity
  shrinks, so copies get denser approaching EOD — where a single
  damage band is most likely to take the final bootstrap and the most
  accumulated map is at stake — without a steep geometric ramp that
  would concentrate flush-stalls and clustering risk in the tape's
  most fragile region.
- A **`checkpoint()`** (§6.1) always writes a non-final bootstrap and
  **resets the staleness counters**; checkpoints (e.g. one before each
  Layer 5 `UNLOAD`, §13.1) thus contribute to the cadence naturally. A
  tape filled over many sessions accumulates a bootstrap at every
  clean session end on top of the in-session floor.

This is content-proportional and EOD-aware (tracking actual value)
while respecting the two constraints a naive geometric curve ignores:
each bootstrap is a synchronous-flush stall with backhitch/wear (§7.7),
and clustered late copies are less independently survivable against a
contiguous band. Much of the structural map is *already* protected
without dense bootstraps — by the replicated sidecar metadata (§5.5)
and the `parity_map`/directory (§5.6.1) — so the bootstrap curve is a
secondary defense tuned for the lost-tail case, not the primary one.
...
of §5.6); replication is their protection. Since non-BOT placement
is content-driven, a reader treats position hints as accelerators:
try BOT, then journal/MAM/catalog/operator hints, then a full
filemark scan (§8.1). There are no required fractional positions.
...
6. OPTIONALLY compute the bootstrap tape-file's own sha256 after
   writing, for optional TapeFileEntry/content sha256 mirror — this content hash is
...
validation). If this bootstrap cannot carry the sidecar directory
inline and needs an external `parity_map`, the writer uses the
§5.6.1 external-map ordering instead: allocate both structural
entries, compute the digest over both, write the `parity_map`, write
the bootstrap, then commit both in one Control/Finish bundle.
...
**Implementation status (2026-06-10).** The current `ParitySink` has not wired
the local-disk spool yet. Completed-epoch sidecars are held in a volatile
RAM-backed `PendingSidecar` queue and are bounded by the object-start capacity
reserve (`epochs_completed_by_object`) before they can be emitted at
`finish_object`.
...
written during emission are committed as one atomic journal bundle:
...
At rem-tar object close:
  ...
  5. 3c commits the object + sidecars + bootstraps as ONE
     `CommittedBundleKind::Object` journal bundle (after all filemarks
     have returned and before any downstream query mirror is updated).
...
journal state where an object is committed but completed sidecars it
generated are absent. Any SQLite/external catalog mirror is updated
only after the journal bundle fsync returns.
```
...
object data block write        continue ONLY if the       hard FormatError; object is
                               remaining reserve still     incomplete; NO journal commit
...
sidecar write                  finish this sidecar tape    if the sidecar cannot be
                               ...
                               and finish the tape          that epoch's objects parity-
                                                           pending in the journal/mirror and
...
journal bundle records the object's *accurate* `parity_state` at
...
visible in derived query mirrors (filemarks don't flush epochs, so a freshly
...
before that object's journal bundle is committed, so the recorded
...
file's filemark is durably written:
  ...
  - NO journal bundle is committed from provisional entries;
  - recovery uses the last COMMITTED state — the journal, or the
    last fully-written on-tape bootstrap whose digest covers only
    its committed prefix (§8.1 prefix validation).
```
...
(final) digest and never reaches the journal. A reader recovering
...
sequence and 3c's journal commit both depend on.
...
```
For each tape file in a bundle (§7.4):
...
Then once, after all the bundle's tape files have returned from (2):
  4. journal.commit_bundle() — durable (fsync) append of the bundle's
                               entries; THIS is the commit point.
```

A "committed tape file" means exactly: it belongs to a bundle whose
`commit_bundle()` (step 4) returned, and the synchronous filemarks of
its tape files (step 2) all returned first. The writer MUST NOT
record a bundle as committed before both its filemarks (step 2) and
its journal fsync (step 4) have returned. The journal append is a
durable local-disk operation, **not** an RDBMS transaction; an
implementation MAY additionally mirror committed bundles to an
external catalog (§10.7), but that mirror is downstream of and never
ahead of the journal. The journal lives on the same trusted local
volume as the parity spool (§7.7 precondition / §10.6). The crash
windows are then all safe:

```
crash between 2 and 4:  tape has valid tape file(s) the journal does
                        not record → restart truncates after the last
                        JOURNAL-committed bundle (the extra files are
                        abandoned). Safe.
crash after 4:          journal and tape agree. Safe.
UNSAFE (forbidden):     recording a bundle as committed before its
                        synchronous filemarks and its journal fsync
                        have returned — a power loss could then claim
                        a tape file that never reached the medium, or
                        a bundle whose commit was never made durable.
```

**Journal durability precondition.** Because the journal fsync is a
commit point with the same status as the synchronous filemark, the
journal MUST live on a volume whose `fsync` is honored. The default
implementation pins it to the parity-spool volume (akash ZFS) and
rejects a network filesystem, tmpfs/ramfs, or a device with a
volatile, non-flushing write cache at session open
(`JournalError`/`SessionOpenError`), mirroring the §6.1.1 drive-
compression precondition. See §10.6.
...
is **journal-driven** (§10), never inferred from `highest_protected_
ordinal` and not simply "the last object." It is identical whether
the prior session ended cleanly or unexpectedly — both rebuild the
in-RAM builder from `journal.load_committed()`.

**Session end, defined in 3c terms (not "unmount").** 3c has no
notion of a physical mount/unmount or cartridge eject — those are
SCSI `LOAD/UNLOAD` / `MOVE MEDIUM` operations owned by Layer 3a and
sequenced by Layer 5 (§13.1). 3c observes only its own write-session
lifecycle, so it defines:

```
CLEAN session end:   checkpoint() or finish() returned success.
                     Tape and journal agree on the committed prefix;
                     a non-final (checkpoint) or final (finish)
                     bootstrap attests it on-tape.
UNCLEAN session end:  anything else — daemon crash, power loss, or
                     the cartridge pulled mid-session — with neither
                     checkpoint() nor finish() having completed.
```

A clean end via `checkpoint()` is resumable (append may continue
later); a clean end via `finish()` is terminal (tape closed forever).
An unclean end takes the resume path below. The two differ only in
speed, not safety: a clean end leaves a fresh on-tape prefix bootstrap
and an agreeing journal, so resume is immediate; an unclean end falls
back to the last journal-committed bundle (fast) or, journal-less, to
a scan from the last on-tape bootstrap (slow). No end state loses
committed data. "Clean unmount" elsewhere in this spec means
precisely "Layer 5 called `checkpoint()` and saw it return before
`UNLOAD`" (§13.1).

**Append point = the trailing filemark of the last
journal-committed *bundle*** on the cartridge (§7.4). A bundle is an
object plus the completed-epoch sidecars it produced plus any
bootstraps/`parity_map` written during emission, made durable in one
`journal.commit_bundle()` call. Example: object X produced sidecars
0–2; the writer wrote object X, its filemark, then sidecars 0 and 1,
and crashed midway through sidecar 2 before the bundle's
`commit_bundle()`. Because the bundle is atomic and never committed,
the append point is after the last *previously* committed bundle —
object X and its partial sidecars are abandoned and the orchestrator
retries object X whole. The append point is never inferred from
`highest_protected_ordinal` and never "the last physical tape file."
...
resume_append_from_committed_prefix(tape_id):
  1. CommittedState = journal.load_committed()  // drops any torn trailing record
  2. N = highest committed tape_file_number in CommittedState.
  3. Position to N; VERIFY it matches the journal entry:
  ...
  6. Rebuild the FilemarkMapBuilder from CommittedState.entries.
  7. Take W and T from CommittedState (highest_protected_ordinal, total_committed_ordinals).
  8. ASSERT the v0.5 committed-state invariant (§7.4):
     ...
     one atomic `CommittedBundleKind::Object` bundle (§7.4). If T − W >=
     data_ordinals_per_epoch, treat it as journal corruption or a
     legacy experimental tape — do NOT silently perform a
     multi-epoch rebuild in production mode.
...
**Tape-only resume (no journal).** The journal is a fast-resume and
commit record, not a hard requirement even for appending — the tape
is self-describing. On a **clean session end via `checkpoint()` or
`finish()`** (§6.1) the writer has written a fresh bootstrap recording
the committed prefix on-tape (§5.6, §7.3), so a later session with no
journal can resume from that bootstrap's attested prefix. On an
**unclean end with no journal**, the writer falls back to scanning from
the widest-scope on-tape bootstrap and **forward-validating bundles**.
The journal, and a `checkpoint()` before each `UNLOAD`, exist to avoid
this slower path. A journal `load_committed()` and a from-tape
reconstruction MUST agree on the committed prefix; a mismatch is a
hard error favoring the on-tape evidence.

Tape-only forward validation is bundle-aware, not simply "accept every
complete object tape file". Starting from the bootstrap-attested
prefix, the scanner walks tape files in order and applies this rule.
The scanner seeds the live partial-epoch state from the attested prefix
before step 1: let `W = directory_scope_highest_protected_ordinal` and
`T = directory_scope_total_data_ordinals` from the bootstrap or
`parity_map` directory; initialize `current_epoch_fill = T - W`. Prefix
validation MUST have already checked `0 <= T - W <
data_shards_per_epoch`, preserving the v1 one-open-epoch invariant.

```
For each candidate object tape file after the attested prefix:
  1. Verify the object tape file is structurally complete:
       trailing filemark present, block_count measured, rem-tar header
       parseable enough to confirm tape_uuid/object_id if readable.
  2. Compute its data_block_count and ordinal range using the current
       FilemarkMapBuilder state.
  3. Compute how many full epochs this object completes:
       completed_epochs = floor((current_epoch_fill + data_block_count)
                                / data_shards_per_epoch)
  4. Require exactly those completed-epoch sidecar tape files to follow
       before accepting the object as committed.
  5. Validate each required sidecar by primary/tail/footer or by a
       bootstrap/parity_map directory entry (§5.5, §5.6.1).
  6. Accept object + required sidecars (+ any intervening Control
       bootstraps/parity_maps) as one reconstructed Object bundle.
  7. If any required sidecar is absent or structurally invalid, stop
       BEFORE the object. The suffix is abandoned and overwritten on
       append. Do not accept the object alone.
```

This preserves the v1 invariant `T − W < data_ordinals_per_epoch`
even when no journal survived. It also prevents a tape-only scanner
from accepting an object whose completed sidecars were lost in a
mid-burst crash.

Step 8's invariant is what bounds restart. Because the
`CommittedBundleKind::Object` bundle is atomic (§7.4), the committed prefix always
...
Crash after all sidecars written, before bundle journal commit:
    Bundle NOT committed. Append after previous committed bundle.
Crash after bundle journal commit:
...
The normal v1 restart invariant means resume rebuilds at most one
partial epoch and usually emits no sidecar before new appends. If a
legacy/experimental tape or tape-only reconstruction causes resume to
emit one or more sidecars, each sidecar follows the same commit
discipline as a sidecar emitted at `finish_object()`:

```
1. write sidecar fixed block(s)
2. write synchronous trailing filemark (§7.7)
3. capture post-filemark physical position
4. append the sidecar TapeFileEntry to a `CommittedBundleKind::ResumeSidecars`
   journal bundle
5. advance CommittedState.highest_protected_ordinal in that bundle
6. let any SQLite/external query mirror recompute affected object
   parity_state values downstream
```

Resume MUST NOT treat rebuilt sidecars as an in-memory-only repair.
They are durable tape files and committed journal entries before any
new object blocks are accepted. The resume API therefore returns the emitted
...
highest-scope copy); then acquire and validate the filemark map. BOT
is the only mandatory fixed position; all later bootstrap copies are
found via optional hints or full filemark scan.
...
pub struct TapeGeometryHint {
    ...
    /// from the journal/query mirror (per-tape block_size), Layer 3a config, or operator config.
    pub configured_block_size: Option<u32>,
    /// Fallback ONLY when configured_block_size is None (e.g. a
    /// foreign/unknown tape): try these finite, ordered sizes, accepting
    /// the first whose block parses as a valid bootstrap (magic +
    /// CRC-64 header check). If empty, use the default list exactly:
    /// [256 KiB, 512 KiB, 1 MiB]. Implementations MUST NOT brute-force
    /// an open-ended size space; any operator extension must be an
    /// explicit finite list of drive-supported fixed block sizes.
    pub candidate_block_sizes: Vec<u32>,
    /// Optional physical positions to probe for bootstrap copies.
    /// These are accelerators only: BOT is always tried, and absence
    /// or staleness of hints falls back to a full filemark scan.
    pub bootstrap_position_hints: Vec<PhysicalPositionHint>,
    /// Whether discovery may perform a full filemark scan if BOT and
    /// hints fail. Catalog-less recovery SHOULD allow this; normal
    /// mounts with a journal may set false for speed.
    pub allow_full_filemark_scan: bool,
}
```

Discovery uses `configured_block_size` if present (Option A — the
normal path: the journal/query mirror or Layer 3a always knows it), else walks
`candidate_block_sizes` validating bootstrap magic and the
CRC-64 header at each candidate (Option B fallback for a tape of
unknown provenance). The fallback candidate set is finite and ordered:
use the configured/catalogued size first when available; otherwise use
`[262144, 524288, 1048576]` bytes (`256 KiB`, `512 KiB`, `1 MiB`)
unless the operator supplies an explicit finite replacement list. Do
not brute-force arbitrary block sizes. Once a bootstrap parses, its
    ...
    -> Result<BootstrapPayload, ParityError>
{
    ...
    // block_size. BOT wins in the common case. Hints are only
    // accelerators; a full filemark scan is the catalog-less fallback.
    for block_size in candidate_block_sizes(&hint) {
        raw.configure_fixed_block_size(block_size)?;   // Layer 3a adapter hook
        if let Ok(bp) = try_read_bootstrap_at(raw, PhysicalPositionHint::bot(), block_size) {
            return validate_bootstrap_read_size(bp, block_size);
        }
        for pos in bootstrap_position_hints(&hint) {
            if let Ok(bp) = try_read_bootstrap_at(raw, pos, block_size) {
                return validate_bootstrap_read_size(bp, block_size);
            }
        }
        if hint.as_ref().map_or(true, |h| h.allow_full_filemark_scan) {
            for bp in full_filemark_scan_for_bootstraps(raw, block_size)? {
                return validate_bootstrap_read_size(bp, block_size);
            }
        }
    }
    ...
    -> Result<BootstrapPayload, ParityError>
{
    ...
    // valid bootstrap copies reachable by BOT + hints + optional full
    // filemark scan, then pick the one whose map digest has the widest
    // scope. Prefer is_final_map=true; otherwise highest sequence /
    // largest map_total_data_ordinals.
    raw.configure_fixed_block_size(block_size)?;
    let mut best: Option<BootstrapPayload> = None;
    for bp in enumerate_valid_bootstraps(raw, &hint, block_size)? {
...
fn validate_bootstrap_read_size(bp: BootstrapPayload, block_size: u32)
    -> Result<BootstrapPayload, ParityError>
{
...
fn enumerate_valid_bootstraps(raw: &mut dyn RawTapeSource,
                              hint: &Option<TapeGeometryHint>,
                              block_size: u32)
    -> Result<Vec<BootstrapPayload>, ParityError>
{
    // BOT first, then hint positions, then full filemark scan if allowed.
    // Duplicate sequences are de-duplicated by (sequence, tape_file_number).
    /* ... */
}
...
on the first try in the common case. Step 2 searches BOT, any
available hints, and if needed the full filemark sequence to find the
highest-scope copy. Because non-BOT placement is content-driven, the
hints are not completeness assumptions. If only the BOT copy survives,
it is used for both roles — and on a tape that was closed cleanly even
the BOT copy is *not* the authoritative one, so step 2 must run whenever
a trusted journal/map is unavailable. (When the journal or a trusted
external mirror supplies the map, step 2 can be skipped — the scheme
from step 1 is all that's needed.)
...
```rust
fn acquire_filemark_map(raw, authoritative_bootstrap, trusted_map)
    -> Result<ScopedFilemarkMap, ParityError>
{
    if let Some((map, watermark)) = trusted_map {
        // A journal or trusted external mirror supplies a complete map;
        // it also supplies CommittedState.highest_protected_ordinal.
...
watermark rule (§7.2.1): when the journal/trusted mirror is present its
watermark is authoritative; tape-only, the validated digest's
...
pub struct ScopedFilemarkMap {
    /// The full reconstructed/trusted map. In the Prefix case it
...
pub enum MapScope {
    /// Trusted journal/external map, or a final-map bootstrap: the
    /// whole tape is described and the watermark is exact.
...
a map but cannot provide one. When the journal/trusted map is gone, the
...
prefix; else a trusted operator-supplied map backup.
```

The bootstrap's `ParityMapReference` is also a structural root of
trust for the referenced control tape file. Its `tape_file_number`,
`block_count`, and `kind=parity_map` classification may be applied
even when the `parity_map` payload cannot be decoded, provided the
scanned tape file has the same `block_count` and was not already
classified as bootstrap or sidecar. Only the directory content inside
the `parity_map` is content-conditional; it is used for directory
overlay only after the payload hash and canonical map digest validate.
...
validation fail for the affected scope — and the fallbacks are a
trusted journal/external map (which never depends on scanning) or another of the
...
- **Bootstrap**: replicated by BOT + content-driven/checkpoint placement (§7.3).
...
### 10.1 Where the filemark map lives

Remanence runs on local files; it depends on no database (§13.1).
The structural **filemark map** has three homes, in order of
authority:

1. **The tape itself — authoritative.** Bootstraps + the
   `parity_map`/`SidecarEpochDirectory` (§5.6, §5.6.1) make a
   cartridge self-describing: a reader with only the tape and the
   spec reconstructs the full map (§8.1). No external store is
   needed to *read* a tape.
2. **A local `TapeFileJournal` — durable resume/commit record.** 3c
   persists each committed bundle through this trait (§10.2). It is
   the commit point (§7.7) and the fast append-resume source (§7.8);
   a few hundred KB to a few MB per tape; no server.
3. **An optional external store — convenience/fleet mirror.** An
   embedded local query catalog (§10.7) and/or a separate
   orchestrator's RDBMS may mirror the map for cross-tape queries.
   This is downstream of the journal and never authoritative.

This is the reverse of earlier drafts, which implied a catalog
(Postgres) was primary and the tape was the fallback. **The tape is
primary.** The map is *not* derivable from object rows alone —
parity-sidecar, bootstrap, and `parity_map` tape files have no
object/file rows yet occupy tape-file numbers and (for sidecars)
define ordinal→parity ranges — so whatever backs the journal records
one structural entry per tape file. The fields of that entry
(whatever the backing) are exactly `TapeFileEntry` (§10.2):

```
per tape file:
  tape_id, tape_file_number, kind ∈ {object, parity_sidecar, bootstrap, parity_map},
  block_count, physical_start_hint?,
  object_id?, first_parity_data_ordinal?,                       -- objects
  epoch_id?, protected_ordinal_start?, protected_ordinal_end_exclusive?,  -- sidecars (half-open)
  canonical_metadata_hash?                                      -- sidecars / parity_map
```

The bootstrap is the **authoritative** parity-scheme source (§5.6).
The journal header MAY repeat `scheme_id`, `k`, `m`, and `S` as a
consistency copy so a stale/wrong journal can be rejected before
resume. On mismatch between bootstrap and journal, the bootstrap wins
and the journal is rejected for resume. The journal's committed
records hold the map (where tape files are and which ordinals they
cover), not the codec definition.

### 10.2 The `TapeFileJournal` trait

3c never opens a database. At each commit point (§7.7) it hands a
durable-append request to a `TapeFileJournal`; that trait is the
*only* persistence surface 3c depends on, and the write session is
constructed with one (`&mut dyn TapeFileJournal` or generic
`J: TapeFileJournal`).
...
pub struct TapeFileEntry {
    ...
    pub block_count: u64,
    pub physical_start_hint: Option<u64>,    // LOCATE hint; advisory, not authoritative
...
/// One atomic commit unit. Not all bundles contain an object:
/// checkpoint() and standalone write_bootstrap() commit Control bundles;
/// resume-generated sidecars commit ResumeSidecars bundles; finish()
/// commits a Finish bundle.
...
pub struct CommittedBundle {
    ...
    pub entries: Vec<TapeFileEntry>,         // ascending tape_file_number
...
pub trait TapeFileJournal {
    ...
    /// Atomically + DURABLY (fsync) append a bundle. Returns only once
    /// durable. All-or-nothing. THIS is the §7.7 commit point.
    fn commit_bundle(&mut self, bundle: &CommittedBundle) -> Result<(), JournalError>;
    /// Replay committed entries (tape_file_number order) to rebuild the
    /// FilemarkMapBuilder on (re)mount (§7.8). MUST drop a torn trailing
    /// record — by definition it was never committed.
...
Three properties matter: **atomic bundle append** (preserves the
`T − W < one epoch` invariant across crashes, §7.4/§7.8); **durable
before return** (the fsync is the commit point, after the bundle's
synchronous filemarks); **torn-record tolerance** (replay's
drop-trailing-torn-record rule *is* §7.8's "append after the last
committed bundle"). Bundle kind is operational metadata only; every
kind obeys the same synchronous-filemark → journal-fsync commit
ordering.

### 10.3 Default backing: `FileTapeFileJournal`

One append-only file per tape, `std` + a CBOR codec + CRC-64 (the
§5.5 CRC-64/XZ), no server:

```
<journal_dir>/<tape_uuid>.remjournal
  [ fixed header once: magic b"REMJRNL\x01", version, tape_uuid,
    block_size, scheme_id+(k,m,S) consistency copy, header_crc64 ]
  [ then append-only bundle records, each:
      u32le record_len ; u8[len] canonical-CBOR(CommittedBundle) ; u64le record_crc64 ]
```

The scheme tuple in the header is a consistency copy, not the
authority; bootstrap discovery supplies the authoritative scheme and
journal replay rejects a mismatch. `commit_bundle`: encode → append
`[len][cbor][crc]` → `fsync` (and
`fsync` the directory on first create) → return. Append-only: only
the trailing record can ever be torn. `load_committed`: validate the
header, walk records, stop at the first record whose length runs past
EOF or whose CRC fails (the crash point), and take the valid prefix.
This yields exactly the set of bundles whose `commit_bundle` returned
— matching §7.8 with no extra machinery. Sizing: ~few hundred bytes
per bundle → well under 1 MB (default scheme) / a few MB
(conservative).

`FileTapeFileJournal::open` is the exclusive append-session
constructor. It holds a non-blocking exclusive flock for the lifetime
of the write session, so two writers cannot append to the same tape
journal concurrently. Read-only projections use
`FileTapeFileJournal::open_shared_for_replay`, which acquires a
non-blocking shared flock and exposes only replay. Shared replay
handles may coexist with one another but intentionally serialize with
an active append writer; a caller that receives lock contention retries
after the tape's live session closes (`JournalError::is_lock_contended`
is the structured check). The read-only replay path never truncates a
torn tail. `FileTapeFileJournalReader::load_committed` consumes the
reader and replays through the already-locked descriptor, so the shared
flock is released before downstream projection work and replay cannot
race a path rename/replacement.

#### Catalog-schema additions (when an external relational backing is used)

The legacy `catalog_tape_files` table from
`3b-catalog-schema-followup.md` is **one possible backing** that an
external catalog (§10.7) may provide for the journal — it is not a 3c
requirement and not required to be Postgres. Its columns are the
`TapeFileEntry` fields above with `kind ∈ {object, parity_sidecar,
bootstrap, parity_map}`; a relational backing additionally enforces
kind-specific CHECK constraints. An external relational backing that
mirrors the journal MUST treat `commit_bundle` as the source of
truth and apply each bundle as one transaction; it is downstream of
and never ahead of the journal fsync. (The deferrable-constraint /
FK-cycle concern that earlier drafts carried applies only to such a
relational backing and is out of scope for the library default.)

#### Parity-protection state (review #1)

Because parity protection lags object close (§7.2.1), a query layer
must track *which* objects are actually protected, not assume "on
tape" means "protected." Whatever backs the journal or downstream query mirror records,
per object, enough to compare its ordinal range to the per-tape
watermark:

```
-- per tape: the monotonic watermark advanced by each emitted sidecar
tape.highest_protected_ordinal           (= CommittedState.highest_protected_ordinal)

-- per object: enough to compare its ordinal range to the watermark
object.first_parity_data_ordinal
object.data_block_count                  -- == object's TapeFileEntry.block_count (§5.6 identity)
object.ordinal_end_exclusive             -- = first_parity_data_ordinal + data_block_count
object.parity_state                      -- ∈ {pending, partial, protected}
```
...
states after the committed journal bundle is mirrored (a range update
on the ordinal columns; see the 3b follow-up for an optional SQL mirror).
...
leaves a `pending`/`partial` tail that the journal and any query
mirror represent honestly.

Objects, parity sidecars, and bootstraps are all
filemark-delimited tape files (§5.1), addressed physically by
`(tape_file_number, block_within_file)`. The map entry's
...
There is no separate "catalog on tape" parity concern in v0.7:
the tape itself is self-describing (bootstraps + directory/`parity_map`
+ filemark map digest), and the local journal is the resume/commit
record. Any SQLite/external catalog is a downstream query mirror. v0.2's
on-tape catalog-replication-with-hint-LBAs model is dropped.

### 10.4 Object header redundancy

rem-tar-v1 keeps per-object identity in pax extended headers
(forward-scan recoverable) and a trailing CBOR manifest; the
trust chain (external object metadata → file_sha256 → manifest_sha256 → per-chunk
...
### 10.5 The unified picture

On a v0.7.2 parity-protected tape:
...
- **Bootstrap tape files** sit outside the ordinal stream;
  protected by BOT + content-driven/checkpoint replication (§5.6, §7.3).
- **The filemark map** ties the three address spaces together. It is
  authoritative **on the tape** (bootstraps + directory), durably
  recorded as you write by the local `TapeFileJournal` (§10.2), and
  optionally mirrored to an external query store (§10.7) — never
...
### 10.6 Journal durability precondition (normative)

The journal's `commit_bundle` fsync is a commit point with the same
status as the synchronous filemark (§7.7), so the journal MUST live
on a volume whose `fsync` is honored. The default
`FileTapeFileJournal` enforces this at session open, mirroring the
§6.1.1 drive-compression precondition:

```
- The journal directory is pinned to the SAME trusted local volume as
  the parity spool (akash ZFS). It is NOT freely relocatable to an
  arbitrary path at runtime.
- Reject at session open (SessionOpenError / JournalError) if the
  journal path is on: a network filesystem (NFS/SMB/etc.), a
  tmpfs/ramfs, or a device advertising a volatile, non-flushing write
  cache. A wrong location is a hard error BEFORE the BOT bootstrap is
  written — never a silent latent corruption.
- On Linux, the default implementation probes `/sys/dev/block` for
  the backing device. `write through` is accepted. `write back` is
  accepted only when the queue reports `fua=1`; missing sysfs data,
  unknown modes, or `write back` without FUA fail closed.
- This FUA requirement is deliberately conservative. Some Linux block
  devices can honor `fsync` through an explicit flush even when they do
  not advertise per-write FUA, but v0.7.2 does not attempt to prove
  that path generically. Operators who hit this rejection need a future
  explicit allowlist or flush-capability policy before the default gate
  is relaxed.
- fsync the file on every commit_bundle; fsync the directory on first
  create.
- There is NO "relaxed durability" / no-fsync mode. The journal write
  is once per object (per bundle), not per block, so it is negligible
  against tape I/O; weakening a commit point is not a tunable.
```

This is validated on the deployed filesystem by a power-loss test
(§14, impl step 11.18c): after a forced power cut, the recovered
committed prefix from `load_committed()` must equal the set of
bundles whose `commit_bundle()` returned, and must agree with a
from-tape reconstruction (§7.8 tape-only resume).

### 10.7 Layer 5 local catalog and the external orchestrator

Two roles must stay separate; neither is a database server inside
Remanence:

**Write-side commit record = the journal (§10.2/§10.3).** Per-tape,
append-only, owned by 3c, the thing whose fsync is the commit point.
Deliberately dumb and robust because it is on the crash-recovery
critical path. It is not a query engine.

**Read-side query index = Layer 5's local catalog: embedded SQLite.**
For cross-tape questions ("which tape holds object X", "list objects
on tape Y", "fleet protection state") Layer 5 keeps an embedded
SQLite catalog (e.g. `rusqlite`, bundled, statically linked) — a
single local file, no server, no separate process. SQLite is chosen
precisely because it satisfies both "no hosted DB / Remanence runs on
local files alone" and "real indexed cross-tape queries" at once. Two
rules keep it honest:

```
1. The SQLite catalog is DERIVED state, never authoritative. There
   MUST be a `rebuild-catalog-from-journals` command (and ultimately
   from-tapes), and a test that wipes the SQLite file and rebuilds it
   equivalently from the per-tape journals. Losing it costs query
   convenience, never data.
2. 3c stays generic over TapeFileJournal and knows nothing about
   SQLite. Layer 5 wires the journal to 3c and SEPARATELY updates its
   SQLite index (by tailing committed bundles). The parity library
   has no database dependency, embedded or otherwise.
```

**External orchestrator (separate software).** Fleet management,
multi-copy placement, and cross-node policy may run a real RDBMS
(Postgres or otherwise). It is **out of process**, possibly on
another host, and is **not** a Remanence dependency: Remanence
neither hosts it nor connects to it for its own durability. The
orchestrator *consumes* Remanence's map via the Layer 5 API; it never
owns it. A relational mirror, if the orchestrator keeps one, is the
downstream backing described in §10.3 — applied per committed bundle,
never ahead of the journal.

### 10.8 Format documentation on tape (long-horizon insurance)
...
bootstraps or journal/query-mirror rows. Place it in the first archive batch's
...
| Bootstrap copies         | BOT + content-driven policy + final | BOT and final are mandatory; intermediate copies follow `BootstrapPlacementPolicy` staleness floors, EOM taper, and minimum-separation guard (§7.3). |
...
pub enum ParityError {
    ...
    #[error("no bootstrap found near probe position {0:?}")]
...
/// Durable-append journal failures (§10.2, §10.6).
#[derive(Debug, thiserror::Error)]
pub enum JournalError {
    #[error("journal I/O: {0}")]
    Io(#[from] std::io::Error),
    #[error("journal header mismatch (tape_uuid / scheme / block_size)")]
    HeaderMismatch,
    #[error("journal encode/decode: {0}")]
    Codec(String),
    #[error("journal volume rejected: {0} (must be a trusted local volume with honored fsync; §10.6)")]
...
| 11.18a | **Commit durability barrier (§7.7).** The `DriveHandleRawSink` adapter implements `write_filemark` as a synchronous flush (IMMED=0 / MTWEOF); a mock `RawTapeSink` models the flush boundary explicitly. Tests assert Layer 5's commit ordering (blocks → sync filemark → position → journal commit) and that the mock rejects a journal commit attempted before the synchronous filemark returned. |
| 11.18b | **Restart / append-after-crash (§7.8).** `resume_append_from_committed_prefix`: reload committed bundles from `TapeFileJournal`, verify + position past the last committed tape file, rebuild the FilemarkMapBuilder, and perform the open-epoch rebuild — re-read object blocks over `[W, T)`, re-accumulate, emit any resume sidecars as `ResumeSidecars` bundles, and load the partial epoch as live `EpochState`. Returns `ResumeAppendResult`. Tests (mock tape, then QuadStor, then one scratch LTO): (1) crash after object filemark, before any sidecar → resume from previous committed bundle; (2) crash after sidecar 1 of an uncommitted bundle → resume from previous committed bundle; (3) crash mid-sidecar → provisional entry ignored; (4) crash after sync filemark, before journal commit → tape has an extra file, journal does not; resume truncates from journal prefix; (5) crash after journal commit → journal and tape agree; (6) `W < T` open-epoch rebuild round-trips to byte-identical accumulators and a clean `finish()` after resume protects everything; (7) unreadable block during rebuild → fail cleanly, Layer 5 falls back to another copy; (8) tape-only resume accepts an object only when its generated sidecars validate. |
...
| 11.8b | **No deferral + object commit bundle (§7.4, §7.8).** Emit all completed sidecars at `finish_object`; `CommittedBundleKind::Object` atomic journal bundle; assert the `T − W < one epoch` invariant on commit and on restart; `SidecarBurstPolicy`; `LargeObjectSidecarClusterRisk` audit. Replaces any volatile-deferral path. |
...
**v0.6 implementation steps (persistence decoupling).**

| Step | Description |
|--|--|
| 11.6b | **`TapeFileJournal` trait + `FileTapeFileJournal` (§10.2/§10.3).** Trait, `TapeFileEntry`/`CommittedBundle`/`CommittedState`, and the append-only single-file default (header + length+CRC-framed bundle records). `commit_bundle` = append + fsync (+ dir fsync on create); `load_committed` = walk + stop at first torn record. Add `open_shared_for_replay` as the non-mutating projection handle; it shares with readers, serializes with append writers, and does not truncate torn tails. Tests: round-trip; torn-trailing-record dropped; bundle atomicity; header mismatch rejected; shared replay locking and no-truncate behavior. 3c is generic over the trait. |
| 11.6c | **Journal durability precondition (§10.6).** Detect and reject network FS / tmpfs / volatile-cache volumes at session open (`JournalError::UntrustedVolume`); on Linux, require either `write through` or `write back` with `fua=1`; pin to the spool volume; no relaxed-durability mode. |
| 11.7b | **Wire 3c to the journal.** `ParitySink` construction takes `&mut dyn TapeFileJournal`; `finish_object`/`finish`/resume call `commit_bundle`; `resume_append_from_committed_prefix` uses `load_committed` (replacing any catalog-row dependency). Tests: object commit bundle is one journal record; resume rebuilds the builder from the journal. |
| 11.18c | **Journal power-loss gate (§10.6, §14).** On the deployed filesystem (akash ZFS), forced power cut → recovered committed prefix equals the set of bundles whose `commit_bundle` returned, and agrees with from-tape reconstruction (§7.8 tape-only resume). |
| 11.L5 | **Layer 5 embedded SQLite catalog (§10.7).** Derived query index updated by tailing committed bundles; `rebuild-catalog-from-journals` command; test that wiping and rebuilding the SQLite file reproduces an equivalent catalog. No server process. |
| 11.NoDB | **No-database build gate (§14).** Full write→unmount→remount→append→read→recover cycle passes with no database process running (journal-only), on mock and scratch LTO. |

**v0.7 implementation steps (checkpoint / session lifecycle).**

| Step | Description |
|--|--|
| 11.8c | **`checkpoint()` + `BootstrapPlacementPolicy` (§6.1, §7.3).** `checkpoint()` (assert no open object; do NOT close the partial epoch; write a non-final prefix bootstrap; commit the in-flight bundle; reset placement counters; return `CheckpointResult`). Content-driven placement (bundles/ordinals floor, EOM taper, min physical separation). Tests: checkpoint → resumable clean end; resume after checkpoint rebuilds the open epoch and a later `finish()` protects everything; placement floor and taper trip at the right points; min-separation defers a tripped floor; checkpoint mid-object is rejected. |
| 11.L5b | **Layer 5 checkpoint-before-UNLOAD (§13.1).** Layer 5 calls `checkpoint()` and observes its return before any SCSI `UNLOAD` of a resumable tape. Test: unload-without-checkpoint leaves an unclean-end tape that still resumes (via journal, then tape-only) with no data loss, only the slower scan. |
...
- Any physical mount/unmount/eject concept — see the session
...
**No database dependency (v0.6).** Remanence depends on no database.
3c depends only on the `TapeFileJournal` trait (§10.2); the default
`FileTapeFileJournal` is `std` + a CBOR codec + CRC-64. Layer 5
persists committed bundles through that same trait and maintains its
*own* local query catalog as **embedded SQLite** (§10.7) — a single
file, statically linked, no server. A hosted RDBMS, if used at all,
belongs to the external orchestrator (separate software, out of
process) and is not a Remanence dependency. The build/test gate
(§14) asserts the full write→unmount→remount→append→read→recover
cycle passes with no database process running.

**Session lifecycle contract (v0.7).** 3c is oblivious to physical
tape mounting: it never issues or observes SCSI `LOAD`/`UNLOAD` or
library `MOVE MEDIUM`. Those are Layer 3a primitives sequenced by
Layer 5. 3c's lifecycle is purely its own API — session open,
`begin_object`/`finish_object`, `write_bootstrap`, `checkpoint`,
`finish` — and "clean vs unclean session end" is defined against
those calls (§7.8), not against any eject event. The obligation that
connects the two layers:

```
Layer 5 MUST call ParitySink::checkpoint() and observe it return
before issuing a SCSI UNLOAD (or library MOVE MEDIUM) on a tape it
intends to resume later. It MUST call finish() before unloading a
tape it is closing permanently.
```

If Layer 5 unloads without a returning `checkpoint()`/`finish()`, the
tape is in the **unclean session-end** state (§7.8): no data is lost
(the journal and on-tape bootstraps still bound the committed prefix),
but the next session pays the slower journal/tape resume scan rather
than resuming from a fresh prefix bootstrap. Layer 5 owns the
mount/unmount state machine and the autochanger; 3c owns only the
durable, resumable on-tape/journal state that a checkpoint produces.
...
**Catalog-less recovery (§8.1).** With no journal/trusted map available, reconstruct the map by scan, validate against the final bootstrap (or referenced `parity_map`) digest, recover a damaged object block; repeat with one damaged sidecar header and prove unrelated epochs still recover.
...
**Power-loss / bundle restart (§7.4, §7.8).** Pull power: before the object filemark; after the object filemark but before the sidecar burst completes; mid-sidecar; after all sidecars but before the bundle journal commit; after the bundle journal commit; mid-burst. On restart, verify the append point is after the previous committed bundle when the current bundle was incomplete, and after the current bundle when it committed; prove the committed-prefix rebuild satisfies `T − W < data_ordinals_per_epoch`.

**Filemark durability (§7.7).** Prove on the target drive that a returned synchronous filemark (IMMED=0 / MTWEOF) is repositionable after power loss, and that the journal bundle is committed only after it returns. Query mirrors may update only after the journal commit.

**Drive compression (§6.1.1, §11.6).** Attempt a parity write with compression externally enabled → 3a detects and disables it before write; if disabling/verifying fails, the write fails before the BOT bootstrap; bootstrap + journal/query mirror record `drive_compression=false`.

**Journal durability + no-database (§10.6, §10.7, §13.1).** On the deployed local filesystem (akash ZFS): forced power cut → `load_committed()` returns exactly the bundles whose `commit_bundle` returned, and agrees with a from-tape reconstruction. The full write→unmount→remount→append→read→recover cycle passes with **no database process running** (journal-only). Journal on a network FS / tmpfs / volatile-cache device is rejected at session open. Layer 5's SQLite catalog rebuilds equivalently from the journals after being wiped.
...
C. Catalog-less recovery works with NO external store present (tape + journal only).
D. Directory overflow uses parity_map; conservative/custom schemes keep directory robustness.
E. No committed v1 state requires restart rebuild of >= one full epoch.
...
O. The full write→read→recover cycle passes with NO database process running (journal-only).
P. journal.commit_bundle's fsync is the commit point; a journal on an untrusted-flush volume is rejected at session open.
Q. journal.load_committed drops a torn trailing record and equals a from-tape reconstruction (power-loss test).
R. Layer 5's SQLite catalog is rebuildable from the journals and is never authoritative.
S. checkpoint() yields a resumable clean end: resume rebuilds the open epoch and a later finish() protects everything; checkpoint mid-object is rejected.
T. A tape UNLOADed without a returning checkpoint()/finish() still resumes with no data loss (journal, then tape-only) — only slower.
```
...
### 15.1 Bootstrap discovery scan cost

The mechanism is now settled, but the operational tuning remains
akash-specific.

Non-BOT bootstrap placement is content-driven (§7.3), not fixed at
fractional tape positions. Therefore a catalog-less reader cannot rely
on `~1/3`, `~2/3`, or `near-EOD` as complete search locations. The
discovery order is:

```
1. Try BOT.
2. Try available hints:
     journal hints, MAM hints if implemented, external catalog hints,
     and operator-provided physical-position hints.
3. If no authoritative bootstrap is found and full-scan is allowed,
   perform a filemark scan: read the first block of each tape file,
   validate bootstrap magic + CRCs, collect all valid bootstraps, and
   select final-map if present else highest-sequence / widest-prefix.
```

Hints are accelerators only. A full filemark scan can be slow on a
large tape, but it is the correct catastrophic catalog-less path and
is covered by the testing plan. The remaining tuning question is how
aggressive `BootstrapPlacementPolicy` should be for akash's object
mix, not how discovery works.
...
- `docs/rem-tar-v1-design.md` v0.9.3 — the default body format;
...
*End of design v0.7.2 (implementation-ready; discovery/journal/parity_map contract polish applied). Comments and corrections welcome — please
