...
**Changes from v0.1:** resolves the review gaps around state-lock
contention/stale-lock behavior, audit framing details, startup replay
ordering, SQLite reconciliation, concrete public type shapes, ENOSPC
handling, clock forward jumps, SQLite corruption, rebuild-reader
concurrency, and per-tape journal ingestion lock contention.
...
Layer 4 must read 3c journals only through the 3c read-only replay
surface (`FileTapeFileJournal::open_shared_for_replay` plus
`load_committed()`, or its successor). It must not reparse
`.remjournal` bytes directly; the on-disk framing, CRC rules,
torn-record handling, and trusted-volume policy remain owned by Layer
3c.

Journal ingestion serializes per tape with live 3c append sessions.
The shared replay handle is non-mutating and can coexist with other
readers, but it intentionally conflicts with the exclusive append
session lock. If opening the replay handle returns lock contention
(`JournalError::is_lock_contended()`), Layer 4 records the tape as
ingestion pending and retries after the live session closes; it must
not take the exclusive append constructor just to build a projection.
`FileTapeFileJournalReader::load_committed()` consumes the replay
handle, so Layer 4 must finish replay before opening its SQLite write
transaction and must not hold a journal flock across index updates.
If a new 3c append session sees journal lock contention because
ingestion is briefly active, Layer 5 returns a retryable "journal
ingestion in progress" open error or backs off and retries within its
...
### 3.1 State Lock Semantics

`state.lock` is an advisory exclusive file lock (`flock(LOCK_EX|LOCK_NB)` on Linux). The file contents are diagnostic only:

```text
pid=<process id>
host_id=<stable host id>
started_at_utc=<RFC3339 UTC>
binary_version=<daemon version>
```

If the lock cannot be acquired, startup returns `StateLockHeld` immediately. Operators may inspect the file, but Layer 4 must not decide staleness from the file contents alone; kernel lock release on process exit is the authority. If the file exists but the lock can be acquired, Layer 4 overwrites the diagnostic contents and continues. If the lock file cannot be opened, written, or fsynced, startup fails closed.
...
Timestamps are RFC3339 UTC. Ordering is by `sequence`, not by wall clock. If the wall clock moves backward, Layer 4 emits `ClockRegressionObserved` but continues with a monotonic sequence. If the wall clock jumps forward by more than the configured tolerance, Layer 4 emits `ClockForwardJumpObserved`; it does not rewrite prior timestamps or change ordering. Operators can correlate the warning with NTP/RTC events.
...
If SQLite contains rows absent from the authoritative source, or if source offsets cannot be trusted, Layer 4 must rebuild the affected projection from scratch before accepting work. The simplest v0.2 implementation may delete and rebuild the full SQLite database. It must not attempt to keep orphan rows and mark them stale.

If SQLite reports internal corruption (`PRAGMA quick_check` failure, page-checksum failure if enabled, malformed schema version, or unreadable WAL), Layer 4 closes SQLite, moves the corrupt database/WAL/SHM files aside with a `.corrupt.<timestamp>` suffix, creates a fresh database, and rebuilds from audit, 3c journals, and tape catalog cache. If rebuild fails, startup fails closed with `IndexCorrupt` or the structured lower-layer cause.

### 6.4 Rebuild Concurrency

`rebuild_index_from_journals()` takes the single-writer state lock and runs inside one SQLite transaction against a shadow set of projection tables. Concurrent readers must either continue using the old committed snapshot or receive a typed `IndexRebuildInProgress` retryable error; they must never observe half-rebuilt projection tables. At commit, Layer 4 atomically swaps the shadow tables into place and advances ingestion offsets.
...
pub struct StateLockGuard { /* owns the locked state.lock file */ }

pub struct CatalogIndex { /* typed SQLite projection owner */ }

pub struct IdempotencyStore { /* audit-backed projection owner */ }
...
`StateHandle` is `Send` but not `Sync`. Layer 5 obtains mutable state access through one owner task; read-only query snapshots may be exposed through typed methods that do not leak raw SQLite connections. Lock acquisition is nonblocking in v0.2; any future wait-with-timeout mode must be explicit config and preserve `StateLockHeld` as the observable contention error.
...
| 4.4 | Implement journal ingestion through `FileTapeFileJournal::open_shared_for_replay` + consuming `load_committed()` into `tapes`, `tape_files`, and object-copy projections; on per-tape lock contention, mark ingestion pending and retry after the append session closes; never hold the replay handle across the SQLite update transaction. |
...
3. Existing stale-looking `state.lock` contents do not block startup when the kernel lock is free.
4. Audit append/replay round-trips records and preserves sequence order.
5. Audit replay drops one torn trailing record but rejects mid-log corruption.
6. Audit rotation chains the next segment to the previous terminal hash.
7. Clock regression and forward jumps emit audit warnings but preserve sequence order.
8. Audit append and SQLite insert ENOSPC return `DiskFull` and enter degraded/read-only mode when an irreversible action may already have completed.
9. SQLite migrations are idempotent and preserve existing rows.
10. SQLite corruption is moved aside and rebuilt from authoritative sources.
11. Indexing a committed 3c bundle creates the expected `tapes` and `tape_files` rows.
12. Wiping SQLite and rebuilding from journals produces an equivalent query index.
13. Readers never observe half-rebuilt tables during `rebuild_index_from_journals()`.
14. Crash after journal commit but before SQLite update is repaired by replay.
15. Crash after SQLite update but before terminal audit append marks the prior session/operation lost by restart.
16. Idempotency retries with the same key/request return the original operation.
17. Idempotency retries with the same key but changed request reject with `IdempotencyConflict`.
18. Catalog cache digest mismatch discards the cache and requires rebuild from tape.
19. No test requires a hosted database process.
...
3. Should `rem-state.sqlite` contain both query catalog and operation/idempotency projections, or split them into two SQLite files? Recommendation: one file for v0.2, with clear table ownership and rebuild rules.
