...
**Archive note:** This staging draft has been promoted into
`docs/layer3c-design.md`, which is now the active v0.7.2 Layer 3c
contract. Keep this file only as the review snapshot that led to the
promotion; do not cite it as normative implementation guidance.
...
| Step | Description |
|--|--|
| 11.6b | **`TapeFileJournal` trait + `FileTapeFileJournal` (§10.2/§10.3).** Trait, `TapeFileEntry`/`CommittedBundle`/`CommittedState`, and the append-only single-file default (header + length+CRC-framed bundle records). `commit_bundle` = append + fsync (+ dir fsync on create); `load_committed` = walk + stop at first torn record. Tests: round-trip; torn-trailing-record dropped; bundle atomicity; header mismatch rejected. 3c is generic over the trait. |
| 11.6c | **Journal durability precondition (§10.6).** Detect and reject network FS / tmpfs / volatile-cache volumes at session open (`JournalError::UntrustedVolume`); pin to the spool volume; no relaxed-durability mode. |
