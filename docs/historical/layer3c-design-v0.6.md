...
**Status:** **v0.6 — implementation-ready; persistence decoupled
from any RDBMS.** This release folds the map-persistence-decoupling
addendum into the spec: Remanence (including Layer 5) runs
end-to-end on local files with **no database process**. 3c persists
committed map entries through a `TapeFileJournal` trait (§10); the
default `FileTapeFileJournal` is a single append-only local file
(`std` + CBOR + CRC-64). The durable, portable source of truth is
the **tape itself** (bootstraps + `parity_map`/directory, §5.6);
the journal is the local resume/commit record; an external
*orchestrator* (separate software, out of process) MAY keep a real
RDBMS but Remanence neither hosts nor requires it. The journal's
fsync is a commit point with the same status as the synchronous
filemark and is pinned to the trusted local volume (§7.7, §10.6).
Layer 5's optional local query catalog is embedded SQLite — a
rebuildable, non-authoritative mirror, never a server (§10.7).
...
the prior session ended in a crash or a planned unmount — both
rebuild the in-RAM builder from `journal.load_committed()`.
...
is self-describing. On a **clean unmount/suspend** the writer writes
a fresh bootstrap recording the committed prefix on-tape (§5.6), so a
later remount with no journal can resume from that bootstrap's
attested prefix. On an **unclean stop with no journal**, the writer
falls back to scanning the committed prefix from the last on-tape
bootstrap and re-validating forward (slow; the journal exists to
avoid this). A journal `load_committed()` and a from-tape
...
The parity *scheme* parameters (k, m, S) are **not** in the journal
or any catalog — they live only in the bootstrap (§5.6). The journal
holds the map (where tape files are and which ordinals they cover),
not the codec.
...
/// One atomic commit unit = the ObjectCommitBundle (§7.4): the object
/// entry + the sidecar entries it generated + any bootstrap/parity_map
/// entries, all made durable together.
...
tape" means "protected." Whatever backs the journal/catalog records,
