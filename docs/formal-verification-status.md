...
the fixed-capacity RAO manifest array/fold proof, the RAO archive composition
proof, the RAO manifest planner-fold and membership-bridge proofs, and the
parity sidecar layout and range-bound bridge proof.
...
| `verif/crc64-xz` | `crates/remanence-crc/src/lib.rs` kernel extraction | CRC-64/XZ constants, reflected bit step, table entry, generated byte update, and the arbitrary-length public `crc64_xz(bytes)` slice loop match the bit-vector fold specification; selected normative vectors are checked. | Sidecar/audit call-site byte ranges are not proved here. |
| `verif/aead-framing` | Pure arithmetic from `crates/remanence-aead/src/{stream,range,inspect}.rs` | V1 (`key_frame_len = 0`) chunk-count validation, payload-frame length, stored-size rounding, ciphertext offsets, plaintext range validation, non-empty range planning, keyless inspect geometry, wrapper equivalence for `expected_stored_size`, and selected fail-closed edges. | V2 key-frame geometry is Rust-test/drift-guard-covered only. Does not prove ChaCha20-Poly1305 security, HKDF, SHA-256, CBOR canonicalization, byte IO, allocation behavior, or full parser/open workflow. |
| `verif/rao-header` | Scalar layout extraction from `crates/remanence-aead/src/header.rs` | V1 RAO AEAD header-core validation, frozen-field emission, parse/serialize round trip for valid header cores, and rejection of bad magic/header length/version/suite/flags/reserved fields. | V2 header/key-frame parsing is Rust-vector/fuzz/test-covered only. Models key id, salt, and object id as validity facts; does not prove exact bytes, hashing, allocation, encryption, or CBOR. |
...
The next formal proof work should expand the RAO archive proof toward byte-level
production behavior:

```text
RAO byte-shape/traversal bridge toward production decode(encode(x)) = x
```

The parity sidecar and CRC scalar/public-loop gaps are now materially narrowed.
The RAO scalar proofs cover header, metadata, manifest, and archive composition,
but they deliberately stop short of exact production CBOR bytes, tar/pax records,
`Vec`/`String` traversal, hashing, encryption, allocation, and IO. Narrowing that
boundary is the highest-value next proof direction.
