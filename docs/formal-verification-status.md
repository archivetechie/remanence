...
planner-fold and membership-bridge proofs, and the parity sidecar layout proof.
...
- Production sidecar byte-buffer construction/parsing beyond the proved scalar
  layout and CRC-window arithmetic.
...
The next formal proof should make the sidecar proof more concrete by bridging
the scalar layout/range proof into production byte-buffer construction and
parsing:

```text
production sidecar Vec/slice writes and reads follow the proved layout ranges
```

The parity sidecar gap is no longer the fixed offset/range arithmetic itself.
It is the bridge from production `Vec<u8>` allocation, `copy_from_slice`
operations, and parser reads into the scalar ranges proved here, plus the
existing CRC-64/XZ arbitrary-slice loop boundary.
