...
array/fold proof, the RAO archive composition proof, the RAO manifest
planner-fold and membership-bridge proofs, and the parity sidecar layout and
range-bound bridge proof.
...
The next formal proof should close the remaining CRC-loop boundary:

```text
crc64_xz(bytes) arbitrary-slice loop equals the byte-fold specification
```

The parity sidecar gap is no longer the fixed offset/range arithmetic or the
fixed buffer-bound bridge for production read/write/CRC windows. Remaining
sidecar risks are actual standard-library `Vec`/slice semantics, variable
index-entry traversal, and the already documented CRC-64/XZ arbitrary-slice
loop boundary. Of those, the CRC loop is the cleanest next proof target because
the scalar kernel proof already exists and the public API is shared across
sidecar and audit call sites.
