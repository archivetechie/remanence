...
Status reflects the proof-bearing state after commit `f11d072`
(`docs: inventory formal verification scope`) plus the RAO header proof added
...
- RAO archive encode/decode round trips beyond the header-core scalar theorem.
...
The next formal proof should continue RAO format correctness incrementally:

```text
decode_metadata(encode_metadata(x)) = x
```

for the deterministic-CBOR metadata core under valid chunk-size and plaintext
size bounds. The header proof establishes the frozen scalar layout; metadata is
the next parser/encoder boundary before the larger manifest/archive
`decode(encode(x)) = x` claim.
