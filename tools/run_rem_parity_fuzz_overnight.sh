...
#
# DO NOT EDIT THIS FILE WHILE A CAMPAIGN IS RUNNING. bash reads a script
# incrementally, so an in-place edit shifts the byte offsets under the running
# interpreter. On 2026-07-30 an edit mid-campaign made the running instance
# die with a syntax error after the fuzzing had completed but before it wrote
# its per-target statistics — the fuzzing results survived only because the
# per-process logs under fuzz/reports/overnight-*/ are written directly.
# If you must change it during a run, copy to a new filename instead.
...
# A long-lived libFuzzer process accumulates RSS independently of the code
# under test: ASan's quarantine plus libFuzzer's own corpus and feature
# bookkeeping. On 2026-07-30 a process crossed a 4 GiB ceiling after 58M
# executions with only ~33 MB live heap, and libFuzzer recorded an OOM
# artifact for the empty input — see
# fuzz/artifacts/rem_parity_bootstrap_structured/README.md. That is a runner
# characteristic, not a parser defect, so the ceiling is set high enough that
# a genuine unbounded allocation in the target is what trips it.
RSS_LIMIT_MB="${FUZZ_RSS_LIMIT_MB:-12288}"
