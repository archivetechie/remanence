#!/usr/bin/env python3
"""Independently re-derive and open REM-OBJECT publication vectors.

This verifier deliberately avoids the Remanence Rust crates. It rebuilds the
REM-OBJECT-TV-P1 and REM-OBJECT-TV-D1 plaintext tar streams, the additional positive
plaintext vectors, and deterministic manifest CBOR. For REM-ENCRYPT 1.0 it derives
...
schedule to open every encrypted vector. The envelope path accepts only the
REM-ENCRYPT 1.0 X-Wing profile.
...
FIXTURES = ROOT / "fixtures" / "rem-object"
TAR_RECORD_SIZE = 512
FORMAT_ID = "rem-object-v1"
SCHEMA_VERSION = "1.0"
...
LABEL_METADATA = b"rem-encrypt-metadata-v1"
LABEL_PAYLOAD = b"rem-encrypt-payload-v1"
WRAP_INFO_PREFIX = b"rem-encrypt-wrap-v1\0"
HPKE_VERSION_LABEL = b"HPKE-v1"
HPKE_XWING_ID = 0x647A
HPKE_KDF_ID = 0x0001
HPKE_AEAD_ID = 0x0003
REM_OBJECT_WRAP_SUITE_XWING = 0x02
REM_OBJECT_KEY_FRAME_MIN_LEN = 1191
REM_OBJECT_KEY_FRAME_MAX_LEN = 16384
REM_OBJECT_KEY_FRAME_MAX_SLOTS = 8
...
PUBLICATION_INCREMENT_FIXTURES = {
    "rem-object-tv-portable-core-only.json",
    "rem-object-tv-nonuser-attribute.json",
    "rem-object-tv-ext-member.json",
    "rem-object-tv-attribute-ext-combined.json",
}
...
def xattr_namespace(name: str) -> str | None:
    """Return the text before the first dot, matching REM-OBJECT Section 4.7.3."""
...
def build_plaintext_with_manifest(
    ...
    """Build canonical REM-OBJECT tar bytes, optionally carrying supplied manifest bytes."""
    chunk_size = options["chunk_size"]
    out = bytearray()
...
def build_plaintext(
    ...
    """Build canonical REM-OBJECT tar bytes and its deterministically encoded manifest."""
...
def positive_plaintext_vector_definitions() -> list[tuple[str, str, dict[str, Any], list[FileSpec]]]:
    ...
    return [
        (
            "rem-object-tv-empty.json",
            "REM-OBJECT-TV-EMPTY",
            vector_options(101, "rem-object-tv-empty", "000000000101"),
            [],
        ),
        (
            "rem-object-tv-empty-file.json",
            "REM-OBJECT-TV-EMPTY-FILE",
            vector_options(102, "rem-object-tv-empty-file", "000000000102"),
            [
                FileSpec(
                    "empty.bin",
        ...
        (
            "rem-object-tv-one-byte.json",
            "REM-OBJECT-TV-ONE-BYTE",
            vector_options(103, "rem-object-tv-one-byte", "000000000103"),
            [
                FileSpec(
                    "one.bin",
        ...
        (
            "rem-object-tv-boundary.json",
            "REM-OBJECT-TV-BOUNDARY",
            vector_options(104, "rem-object-tv-boundary", "000000000104"),
            [
        ...
        (
            "rem-object-tv-paths.json",
            "REM-OBJECT-TV-PATHS",
            vector_options(105, "rem-object-tv-paths", "000000000105"),
            [
        ...
        (
            "rem-object-tv-metadata.json",
            "REM-OBJECT-TV-METADATA",
            vector_options(106, "rem-object-tv-metadata", "000000000106"),
            [
                FileSpec(
                    "meta/mtime.txt",
        ...
        (
            "rem-object-tv-order.json",
            "REM-OBJECT-TV-ORDER",
            vector_options(107, "rem-object-tv-order", "000000000107"),
            [
        ...
        (
            "rem-object-tv-manifest.json",
            "REM-OBJECT-TV-MANIFEST",
            vector_options(108, "rem-object-tv-manifest", "000000000108"),
            [
        ...
        (
            "rem-object-tv-nonregular.json",
            "REM-OBJECT-TV-NONREGULAR",
            vector_options(109, "rem-object-tv-nonregular", "000000000109"),
            [
        ...
        (
            "rem-object-tv-hardlinks.json",
            "REM-OBJECT-TV-HARDLINKS",
            vector_options(110, "rem-object-tv-hardlinks", "000000000110"),
            [
                FileSpec(
                    "primary.txt",
        ...
        (
            "rem-object-tv-xattrs.json",
            "REM-OBJECT-TV-XATTRS",
            vector_options(111, "rem-object-tv-xattrs", "000000000111"),
            [
                FileSpec(
                    "tagged.txt",
        ...
        (
            ...
            vector_options(112, "rem-object-tv-portable-core-only", "000000000112"),
            [
                FileSpec(
        ...
        (
            ...
            vector_options(113, "rem-object-tv-nonuser-attribute", "000000000113"),
            [
                FileSpec(
        ...
        (
            ...
            {
                **vector_options(114, "rem-object-tv-ext-member", "000000000114"),
        ...
        (
            ...
            vector_options(115, "rem-object-tv-attribute-ext-combined", "000000000115"),
            [
                FileSpec(
...
def fixture_json(vector_id: str, options: dict[str, Any], files: list[FileSpec]) -> dict[str, Any]:
    ...
    return {
        ...
        "independent_rederivation": "verified by tools/verify_rem_object_vectors_independent.py",
        "inputs": {
            **options,
...
def check_plaintext_interop(vectors: list[PlaintextVector], allow_missing_bsdtar: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rem-object-plaintext-interop-") as tmp_name:
    ...
    print(f"verified REM-OBJECT plaintext interop for {vector_ids} with {readers}")
...
def parse_encrypted_header(stored: bytes) -> EncryptedHeader:
    if len(stored) < REM_OBJECT_HEADER_LEN:
        raise AssertionError("encrypted object is shorter than the REMO header")
    ...
    if not REM_OBJECT_KEY_FRAME_MIN_LEN <= key_frame_len <= REM_OBJECT_KEY_FRAME_MAX_LEN:
...
def parse_key_frame(
    ...
    if not REM_OBJECT_KEY_FRAME_MIN_LEN <= len(encoded) <= REM_OBJECT_KEY_FRAME_MAX_LEN:
        raise AssertionError(f"{vector_id}: key frame length is outside bounds")
    enc_len = XWING_CIPHERTEXT_LEN
    assert_eq(encoded[:4], b"REMK", f"{vector_id} key frame magic")
    slot_count = encoded[4]
    if not 1 <= slot_count <= REM_OBJECT_KEY_FRAME_MAX_SLOTS:
...
def hpke_unwrap_dek(
    ...
    info = (
        ...
        + slot.recipient_epoch_id
        + bytes([slot.slot_index, 1, wrap_suite])
    )
    key, base_nonce = hpke_base_context(shared_secret, HPKE_XWING_ID, info)
...
def verify_xwing_kats(kat_directory: pathlib.Path) -> None:
    ...
    assert_eq(public_key, wrap["pk"], "REM-OBJECT X-Wing wrap public key")
    enc, sender_secret = xwing_encapsulate(
        public_key,
        wrap["encapsulation_randomness"],
    )
    assert_eq(enc, wrap["enc"], "REM-OBJECT X-Wing wrap ciphertext")
    assert_eq(sender_secret, wrap["ss"], "REM-OBJECT X-Wing wrap shared secret")
    recipient_secret, recipient_public = xwing_decapsulate(
        wrap["seed"],
        wrap["enc"],
    )
    assert_eq(recipient_public, public_key, "REM-OBJECT wrap decapsulation public key")
    assert_eq(
        recipient_secret,
        sender_secret,
        "REM-OBJECT wrap decapsulation shared secret",
    )

    object_id = wrap["object_id"].decode("utf-8")
    slot_index = int.from_bytes(wrap["slot_index"], "big")
    assert_eq(len(wrap["slot_index"]), 1, "REM-OBJECT wrap slot_index length")
    info = (
        WRAP_INFO_PREFIX
        + object_id_field(object_id)
        + wrap["recipient_epoch_id"]
        + bytes([slot_index, 1, REM_OBJECT_WRAP_SUITE_XWING])
    )
    ...
    assert_eq(dek, wrap["dek"], "REM-OBJECT X-Wing wrapped DEK")
    print(
        "verified draft-10 X-Wing and deterministic REM-OBJECT wrap KATs "
...
def open_encrypted_with_generic_crypto(
    ...
    stored_size = round_up(footer_offset + len(REM_OBJECT_FOOTER), header.chunk_size)
    assert_eq(len(stored), stored_size, f"{vector_id} stored_size")
    assert_eq(stored[footer_offset : footer_offset + len(REM_OBJECT_FOOTER)], REM_OBJECT_FOOTER, f"{vector_id} footer")
    fill = stored[footer_offset + len(REM_OBJECT_FOOTER) :]
...
def current_xwing_expected(
    ...
    key_frame = stored[REM_OBJECT_HEADER_LEN : REM_OBJECT_HEADER_LEN + header.key_frame_len]
    slots = parse_key_frame(vector_id, key_frame, header.wrap_suite)
    recipients_by_epoch = {
...
def write_current_xwing_fixture_pins(
    ...
    """Write current REM-ENCRYPT 1.0 envelope pins into staged fixtures."""
    ...
    e2_path = fixture_directory / "rem-object-tv-e2.json"
    ...
    e2["independent_open_rederivation"] = (
        "verified by tools/verify_rem_object_vectors_independent.py with "
        f"{KYBER_PY_DISTRIBUTION}=={KYBER_PY_VERSION}"
    )
    update_recipients(e2, "REM-OBJECT-TV-E2")
    e2["expected"] = current_xwing_expected(
        "REM-OBJECT-TV-E2",
        (object_directory / "rem-object-tv-e2.rem-object").read_bytes(),
        e2["inputs"],
    )

    d1_path = fixture_directory / "rem-object-tv-d1.json"
    ...
    update_recipients(d1, "REM-OBJECT-TV-D1 encrypted")
    d1["expected"]["encrypted"] = current_xwing_expected(
        "REM-OBJECT-TV-D1 encrypted",
        (object_directory / "rem-object-tv-d1-encrypted.rem-object").read_bytes(),
        d1["inputs"],
    )
...
def check_cross_layer_binding_and_range_vectors(
    ...
    assert_eq(object_bytes, p1_plaintext, "object-id-36 REM-OBJECT-TV-P1 bytes")
    assert_eq(
    ...
    range_root = publication_root / "rem-object"
    positive_root = (
...
def p1_file_specs() -> list[FileSpec]:
    """Return the canonical REM-OBJECT-TV-P1 source entries."""
    return [
        FileSpec(
            "a/hello.txt",
        ...
        FileSpec(
            ...
            bytes(index % 256 for index in range(5000)),
        ),
    ]


def d1_file_spec() -> FileSpec:
    """Return the canonical REM-OBJECT-TV-D1 source entry."""
    return FileSpec(
        "v.bin",
        "00000000-0000-4000-8000-000000000012",
        bytes(index % 256 for index in range(262145)),
    )


def core_plaintext_expected(
    options: dict[str, Any],
    files: list[FileSpec],
) -> tuple[dict[str, Any], bytes, dict[str, Any]]:
    """Derive the common pinned fields for a core plaintext vector."""
    plaintext, layout = build_plaintext(options, files)
    expected = {
        "chunk_size": options["chunk_size"],
        "stored_size_bytes": len(plaintext),
    ...
    return expected, plaintext, layout


def write_core_plaintext_fixture_pins(fixture_directory: pathlib.Path) -> None:
    """Write independently derived REM-OBJECT-TV-P1 and D1 plaintext pins."""
    p1_path = fixture_directory / "rem-object-tv-p1.json"
    p1 = load(fixture_directory, p1_path.name)
    p1_files = p1_file_specs()
    p1_expected, _p1_plaintext, p1_layout = core_plaintext_expected(
        p1["inputs"],
        p1_files,
    )
    p1_expected.update(
        {
            "file_payloads": [
                ...
                for spec in p1_files
            ],
            "file_layouts": [layout_json(layout) for layout in p1_layout["files"]],
            "manifest_layout": layout_json(p1_layout["manifest"]),
        }
    )
    p1["expected"] = p1_expected

    d1_path = fixture_directory / "rem-object-tv-d1.json"
    d1 = load(fixture_directory, d1_path.name)
    d1_file = d1_file_spec()
    d1_expected, _d1_plaintext, d1_layout = core_plaintext_expected(
        d1["inputs"],
        [d1_file],
    )
    d1_expected.update(
        {
            "file_payload": {
                "path": d1_file.path,
                "size_bytes": d1_file.size_bytes,
                "sha256": hx(d1_file.file_sha256 or b""),
            },
            "file_layout": layout_json(d1_layout["files"][0]),
            "manifest_layout": layout_json(d1_layout["manifest"]),
        }
    )
    d1["expected"]["plaintext"] = d1_expected

    for path, fixture in ((p1_path, p1), (d1_path, d1)):
        path.write_text(
            json.dumps(fixture, indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
            newline="\n",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="regenerate the additional REM-OBJECT 13.1 plaintext fixture JSON files",
    )
    parser.add_argument(
        "--write-core-plaintext-fixtures",
        action="store_true",
        help="regenerate the REM-OBJECT-TV-P1 and D1 plaintext fixture pins",
    )
    parser.add_argument(
        "--write-publication-increment-fixtures",
        action="store_true",
        help="write only the four REM-OBJECT metadata/extension publication fixtures",
    )
    parser.add_argument(
        "--export-directory",
        type=pathlib.Path,
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        default=FIXTURES,
        help="directory containing the REM-OBJECT fixture manifests",
    )
    parser.add_argument(
        "--encrypted-object-directory",
        type=pathlib.Path,
        default=FIXTURES / "objects",
        help="directory containing the REM-OBJECT-TV-E2 and encrypted REM-OBJECT-TV-D1 objects",
    )
    parser.add_argument(
        "--kat-directory",
        type=pathlib.Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write_new_plaintext_fixtures:
        ...
        print("wrote additional REM-OBJECT 13.1 positive plaintext fixtures")
        return 0
    if args.write_core_plaintext_fixtures:
        write_core_plaintext_fixture_pins(fixture_directory)
        print("wrote REM-OBJECT-TV-P1 and D1 plaintext fixture pins")
        return 0
    ...
    for spec, expected in zip(p1_files, p1["expected"]["file_payloads"], strict=True):
        assert_eq(spec.path, expected["path"], f"REM-OBJECT-TV-P1 {spec.path} path")
        assert_eq(spec.size_bytes, expected["size_bytes"], f"REM-OBJECT-TV-P1 {spec.path} size_bytes")
        assert_eq(hx(spec.file_sha256), expected["sha256"], f"REM-OBJECT-TV-P1 {spec.path} sha256")
    p1_plaintext, p1_layout = build_plaintext(p1["inputs"], p1_files)
    check_plaintext("REM-OBJECT-TV-P1", p1, p1_plaintext, p1_layout, p1["expected"])
    check_layouts("REM-OBJECT-TV-P1", p1_layout["files"], p1["expected"]["file_layouts"])
    check_layouts("REM-OBJECT-TV-P1 manifest", [p1_layout["manifest"]], [p1["expected"]["manifest_layout"]])

    d1_file = d1_file_spec()
    d1_expected_payload = d1["expected"]["plaintext"]["file_payload"]
    assert_eq(d1_file.path, d1_expected_payload["path"], "REM-OBJECT-TV-D1 payload path")
    assert_eq(d1_file.size_bytes, d1_expected_payload["size_bytes"], "REM-OBJECT-TV-D1 payload size_bytes")
    assert_eq(hx(d1_file.file_sha256), d1_expected_payload["sha256"], "REM-OBJECT-TV-D1 payload sha256")
    d1_plaintext, d1_layout = build_plaintext(d1["inputs"], [d1_file])
    d1_expected_plain = d1["expected"]["plaintext"]
    check_plaintext("REM-OBJECT-TV-D1 plaintext", d1, d1_plaintext, d1_layout, d1_expected_plain)
    check_layouts("REM-OBJECT-TV-D1", d1_layout["files"], [d1_expected_plain["file_layout"]])
    check_layouts("REM-OBJECT-TV-D1 manifest", [d1_layout["manifest"]], [d1_expected_plain["manifest_layout"]])

    p1_vector = PlaintextVector(
        vector_id="REM-OBJECT-TV-P1",
    ...
    if args.rust_object_directory is not None:
        for vector in increment_vectors.values():
            rust_path = (
                args.rust_object_directory / f"{vector.vector_id.lower()}.rem-object"
            )
            assert_eq(
    ...
    if args.export_directory is not None:
        ...
        exports = {
            "rem-object-tv-p1.rem-object": p1_plaintext,
            "rem-object-tv-d1-plaintext.rem-object": d1_plaintext,
            "rem-object-tv-e2.rem-object": e2_stored,
            "rem-object-tv-d1-encrypted.rem-object": d1_encrypted_stored,
        }
        exports.update(
            {
                f"{vector.vector_id.lower()}.rem-object": vector.plaintext
                for vector in extra_plaintext_vectors
            }
        )
        for filename, payload in sorted(exports.items()):
            (export_directory / filename).write_bytes(payload)
        print(f"exported {len(exports)} positive REM-OBJECT byte streams to {export_directory}")
    ...
    print(
        "verified REM-OBJECT-TV-E2 and REM-OBJECT-TV-D1 REM-ENCRYPT OPEN, "
        "REM-OBJECT-TV-P1 and REM-OBJECT-TV-D1 "
        "plaintext, additional REM-OBJECT positives, and requested cross-layer "
...
if __name__ == "__main__":
    try:
        ...
        print(f"independent REM-OBJECT vector verification failed: {exc}", file=sys.stderr)
        raise
