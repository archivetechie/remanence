...
fn append_and_project_audit_locked(
    index: &mut CatalogIndex,
    audit_dir: &Path,
    audit_fsync: bool,
    input: ProjectedAuditInput<'_>,
...
fn manual_finalize_finished_audit_matches(
    record: &AuditRecord,
    tape_uuid: [u8; 16],
...
fn audit_subject_matches_tape(record: &AuditRecord, tape_uuid: [u8; 16]) -> bool {
...
fn tape_sealed_audit_matches(record: &AuditRecord, tape_uuid: [u8; 16]) -> bool {
    record.actor == AuditActor::System
...
fn replay_checkpoint_journal_projections_with_audit(
    index: &mut CatalogIndex,
    checkpoint_dir: &Path,
    audit_dir: &Path,
    audit_append_lock: &Arc<std::sync::Mutex<()>>,
...
fn catalog_integrity_basis(basis: ScanIntegrityBasis) -> pb::IntegrityBasis {
    match basis {
...
mod tests {
    ...
    use remanence_format_driver::{
        ArchiveEventSink, ArchiveReader, FileDataSink, FileId, FileStreamReport,
        ForeignFormatAdapter, FormatCapabilities, FormatDescriptor, ScanReport, StreamReport,
    };
...
";
    ...
    fn startup_replay_restores_pending_automatic_finalization_through_replica_c() {
        ...
        let audit = FileAuditLog::replay(temp.path().join("audit"))
            .expect("replay startup sealing evidence");
        assert_eq!(
            audit
                .iter()
                .filter(|record| {
