//! Standalone, catalogless recovery for REM-OBJECT recipient-envelope objects.
...
#[command(
    name = "rem-recover",
    about = "Recover plaintext members from one REM-OBJECT encrypted object without a catalog or daemon"
)]
struct Args {
    /// Complete stored REM-OBJECT object bytes.
    #[arg(long, value_name = "PATH")]
    object: PathBuf,

    /// REMP recipient private-key file for the envelope object.
    #[arg(long, value_name = "PATH")]
    private_key: PathBuf,
...
fn write_recovery_summary(
    ...
    writeln!(
        out,
        "recovered {} files ({} bytes) from REM-OBJECT v{} object {}",
...
mod tests {
    ...
    fn recovers_encrypted_members_and_reports_epoch_mismatch() {
        ...
        let object = temp.path().join("object.rem-object");
    ...
    fn recovery_default_restores_user_xattrs_and_reports_policy_skips() {
        ...
        spec.xattrs
            .insert("user.rem_recover".to_string(), b"kept".to_vec());
        spec.xattrs.insert(
        ...
        let temp = tempfile::tempdir().unwrap();
        let object = temp.path().join("object.rem-object");
        let private_key = temp.path().join("recovery.remp");
        let out = temp.path().join("out");
        fs::create_dir(&out).unwrap();
        fs::write(&object, sealed).unwrap();
        ...
        assert_eq!(
            xattr::get(out.join("tagged.txt"), "user.rem_recover").unwrap(),
            Some(b"kept".to_vec())
        );
