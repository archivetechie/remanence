//! Verifies current REM-OBJECT and REM-ENCRYPT publication vectors.
...
use remanence_aead::{
    ...
    RecipientPrivateKey, RecipientPublicKey, RemObjectAeadError, SealOptions,
    CHACHA20POLY1305_TAG_LEN, REM_OBJECT_WRAP_SUITE_XWING,
};
...
fn current_xwing_encrypted_vectors() -> (Vec<u8>, Vec<u8>) {
    ...
    assert_eq!(
        e2_first, e2_second,
        "REM-OBJECT-TV-E2 regenerates byte-exactly"
    );
    ...
    assert_eq!(
        d1_first, d1_second,
        "REM-OBJECT-TV-D1 encrypted half regenerates byte-exactly"
    );

    for (name, bytes) in [
        ("REM-OBJECT-TV-E2", &e2_first),
        ("REM-OBJECT-TV-D1 encrypted", &d1_first),
    ] {
        let inspected = inspect_bytes(bytes)
            .unwrap_or_else(|error| panic!("{name} X-Wing envelope inspects: {error}"));
        assert_eq!(
            inspected.header.wrap_suite, REM_OBJECT_WRAP_SUITE_XWING,
            "{name} emits only the X-Wing discriminator"
        );
        assert!(
            (remanence_aead::REM_OBJECT_KEY_FRAME_MIN_LEN
                ..=remanence_aead::REM_OBJECT_KEY_FRAME_MAX_LEN)
                .contains(&(inspected.header.key_frame_len as usize)),
            "{name} key frame is inside the REM-ENCRYPT 1.0 bounds"
        );
    }
...
fn publication_increment_vectors() -> Vec<(&'static str, RemTarObjectOptions, Vec<TestEntry>)> {
    ...
    let mut extension_options = vector_options(114, "rem-object-tv-ext-member", "000000000114");
...
fn increment_fixture(filename: &str) -> (&'static str, &'static str) {
    match filename {
        "rem-object-tv-portable-core-only.rem-object" => (
            include_str!("../../../fixtures/rem-object/rem-object-tv-portable-core-only.json"),
            "REM-OBJECT-TV-PORTABLE-CORE-ONLY",
        ),
        "rem-object-tv-nonuser-attribute.rem-object" => (
            include_str!("../../../fixtures/rem-object/rem-object-tv-nonuser-attribute.json"),
            "REM-OBJECT-TV-NONUSER-ATTRIBUTE",
        ),
        "rem-object-tv-ext-member.rem-object" => (
            include_str!("../../../fixtures/rem-object/rem-object-tv-ext-member.json"),
            "REM-OBJECT-TV-EXT-MEMBER",
        ),
        "rem-object-tv-attribute-ext-combined.rem-object" => (
            include_str!("../../../fixtures/rem-object/rem-object-tv-attribute-ext-combined.json"),
...
#[test]
fn rem_object_tv_p1_matches_fixture_manifest() {
    let fixture = fixture(include_str!(
        "../../../fixtures/rem-object/rem-object-tv-p1.json"
    ));
    let inputs = field(&fixture, "inputs");
    let expected = field(&fixture, "expected");
    let hello = b"hello, rem-object\n";
...
#[test]
fn rem_encrypt_xwing_envelopes_are_deterministic_and_range_safe() {
    let p1_plaintext = build_p1_plaintext();
    let (e2_primary, _) = e2_recipient_pair();
    let (e2_first, d1_first) = current_xwing_encrypted_vectors();
    let e2_inspect = inspect_bytes(&e2_first).expect("REM-ENCRYPT 1.0 E2 envelope inspects");
    assert_eq!(
        e2_inspect.header.wrap_suite, REM_OBJECT_WRAP_SUITE_XWING,
        "current format integration emits only the X-Wing discriminator"
    );
    let (e2_opened, _) =
        open_to_vec(&e2_first, &e2_primary).expect("REM-ENCRYPT 1.0 E2 envelope opens");
    assert_eq!(e2_opened, p1_plaintext);

    let d1_plaintext = build_d1_plaintext();
    let (d1_primary, _) = d1_recipient_pair();
    let d1_fixture = fixture(include_str!(
        "../../../fixtures/rem-object/rem-object-tv-d1.json"
    ));
    ...
    .expect("REM-OBJECT-TV-D1 manifest range in the final object chunk authenticates");
    let d1_inspect = inspect_bytes(&d1_first).expect("REM-OBJECT-TV-D1 encrypted object inspects");
    assert_eq!(manifest_range, d1_manifest);
    assert_eq!(
        range_report.first_chunk,
        Some(d1_inspect.chunk_count - 1),
        "REM-OBJECT-TV-D1 manifest range reaches the true final object chunk"
    );
    assert_eq!(range_report.chunk_count, 1);

    let (d1_opened, d1_open) = open_to_vec(&d1_first, &d1_primary).expect("REM-OBJECT-TV-D1 opens");
    assert_eq!(d1_opened, d1_plaintext);
    let d1_key_frame = d1_open
        .key_frame
        .serialize()
        .expect("REM-OBJECT-TV-D1 key frame serializes");
    let d1_keys = derive_keys(
        ...
        &d1_open
            ...
            .expect("REM-OBJECT-TV-D1 header hash derives"),
    )
    .expect("REM-OBJECT-TV-D1 payload key derives");
    ...
    let final_chunk_offset = usize::try_from(
        ...
        .expect("REM-OBJECT-TV-D1 final chunk offset derives"),
    )
    .expect("REM-OBJECT-TV-D1 final chunk offset fits usize");
    ...
    assert!(
        matches!(wrong_finality, RemObjectAeadError::AeadAuthenticationFailed),
        "expected AeadAuthenticationFailed, got {wrong_finality:?}"
    );
}

#[test]
fn rem_object_publication_objects_regenerate_byte_exactly() {
    ...
    if let Some(directory) = std::env::var_os("REM_OBJECT_VECTOR_EXPORT_DIR") {
        let directory = PathBuf::from(directory);
        fs::create_dir_all(&directory).unwrap();
        fs::write(directory.join("rem-object-tv-e2.rem-object"), e2).unwrap();
        fs::write(
            directory.join("rem-object-tv-d1-encrypted.rem-object"),
            d1_encrypted,
        )
        .unwrap();
...
#[test]
fn checked_in_rem_encrypt_vectors_match_the_current_generator() {
    let (generated_e2, generated_d1) = current_xwing_encrypted_vectors();
    let e2 = fixture(include_str!(
        "../../../fixtures/rem-object/rem-object-tv-e2.json"
    ));
    let checked_in_e2 = fs::read(fixture_object_path("rem-object-tv-e2.rem-object"))
        .expect("REM-OBJECT-TV-E2 fixture exists");
    assert_eq!(checked_in_e2, generated_e2);
    assert_eq!(
        sha256_hex(&checked_in_e2),
        str_field(field(&e2, "expected"), "stored_digest")
    );

    let d1 = fixture(include_str!(
        "../../../fixtures/rem-object/rem-object-tv-d1.json"
    ));
    let checked_in_d1 = fs::read(fixture_object_path("rem-object-tv-d1-encrypted.rem-object"))
        .expect("REM-OBJECT-TV-D1 encrypted fixture exists");
    assert_eq!(checked_in_d1, generated_d1);
    assert_eq!(
        sha256_hex(&checked_in_d1),
        str_field(field(field(&d1, "expected"), "encrypted"), "stored_digest")
    );
}

#[test]
fn rem_object_tv_d1_plaintext_matches_fixture_manifest() {
    let fixture = fixture(include_str!(
        "../../../fixtures/rem-object/rem-object-tv-d1.json"
    ));
    let inputs = field(&fixture, "inputs");
...
#[test]
fn rem_object_tv_empty_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-empty.json"),
        "REM-OBJECT-TV-EMPTY",
...
#[test]
fn rem_object_tv_empty_file_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-empty-file.json"),
...
#[test]
fn rem_object_tv_one_byte_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-one-byte.json"),
...
#[test]
fn rem_object_tv_boundary_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-boundary.json"),
...
#[test]
fn rem_object_tv_paths_matches_fixture_manifest() {
    let long_path = format!("long/{}.bin", "a".repeat(102));
    let inline_100 = format!("inline-{}", "b".repeat(93));
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-paths.json"),
        "REM-OBJECT-TV-PATHS",
...
#[test]
fn rem_object_tv_metadata_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-metadata.json"),
...
#[test]
fn rem_object_tv_order_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-order.json"),
        "REM-OBJECT-TV-ORDER",
...
#[test]
fn rem_object_tv_manifest_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-manifest.json"),
...
#[test]
fn rem_object_tv_hardlinks_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-hardlinks.json"),
...
#[test]
fn rem_object_tv_xattrs_matches_fixture_manifest() {
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-xattrs.json"),
...
#[test]
fn rem_object_publication_increment_matches_fixture_manifests() {
...
#[test]
fn rem_object_publication_extension_members_reemit_byte_identically() {
...
#[test]
fn rem_object_tv_nonregular_matches_fixture_manifest() {
    let long_target = format!("targets/{}", "x".repeat(120));
    assert_plaintext_vector_fixture(
        include_str!("../../../fixtures/rem-object/rem-object-tv-nonregular.json"),
