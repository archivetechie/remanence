...
use remanence_aead::{
    ...
    seal_to_vec, DataEncryptionKey, EnvelopeSealOptions, KeyFrame, RecipientPrivateKey,
    RecipientPublicKey, RecipientSlot, RemObjectAeadError, RemObjectHeader, RemObjectMetadata,
    SealOptions, REM_OBJECT_FOOTER, REM_OBJECT_KEY_FRAME_MAX_LEN, REM_OBJECT_KEY_FRAME_MIN_LEN,
    XWING_CIPHERTEXT_LEN,
};
use remanence_format::{
    plan_rem_tar_object, read_encrypted_rem_object, read_rem_tar_object, stream_rem_tar_object,
...
fn aead_error_name(error: &RemObjectAeadError) -> &'static str {
    match error {
        RemObjectAeadError::InvalidMagicBytes => "InvalidMagicBytes",
        RemObjectAeadError::InvalidHeaderLength => "InvalidHeaderLength",
        RemObjectAeadError::UnsupportedFormatVersion => "UnsupportedFormatVersion",
        RemObjectAeadError::InvalidSuite => "InvalidSuite",
        RemObjectAeadError::InvalidWrapSuite => "InvalidWrapSuite",
        RemObjectAeadError::InvalidKeyFrameLength => "InvalidKeyFrameLength",
        RemObjectAeadError::InvalidKeyFrame => "InvalidKeyFrame",
        RemObjectAeadError::HpkeFailed => "HpkeFailed",
        RemObjectAeadError::RecipientEpochMismatch => "RecipientEpochMismatch",
        RemObjectAeadError::InvalidChunkSize => "InvalidChunkSize",
        RemObjectAeadError::ReservedBytesNotZero => "ReservedBytesNotZero",
        RemObjectAeadError::InvalidSalt => "InvalidSalt",
        RemObjectAeadError::MetadataFrameLengthInvalid => "MetadataFrameLengthInvalid",
        RemObjectAeadError::InvalidObjectIdField => "InvalidObjectIdField",
        RemObjectAeadError::InvalidInput(_) => "InvalidInput",
        RemObjectAeadError::UnexpectedEof => "UnexpectedEof",
        RemObjectAeadError::MissingFinalChunk => "MissingFinalChunk",
        RemObjectAeadError::InvalidFooter => "InvalidFooter",
        RemObjectAeadError::TrailingData => "TrailingData",
        RemObjectAeadError::FillNotZero => "FillNotZero",
        RemObjectAeadError::AeadAuthenticationFailed => "AeadAuthenticationFailed",
        RemObjectAeadError::InvalidCborEncoding => "InvalidCborEncoding",
        RemObjectAeadError::MissingRequiredMetadataField => "MissingRequiredMetadataField",
        RemObjectAeadError::InvalidMetadataField => "InvalidMetadataField",
        RemObjectAeadError::SaltDerivationMismatch => "SaltDerivationMismatch",
        RemObjectAeadError::PlaintextDigestMismatch => "PlaintextDigestMismatch",
        RemObjectAeadError::PlaintextSizeMismatch => "PlaintextSizeMismatch",
        RemObjectAeadError::SizeOverflow => "SizeOverflow",
        RemObjectAeadError::Io(_) => "Io",
...
fn base_plaintext_archive() -> PlaintextArchive {
    ...
    let mut sink = VecBlockSink::new();
    let layout = write_rem_tar_object(&mut sink, &options, &files).expect("base REM-OBJECT writes");
...
fn hardlink_plaintext_archive() -> PlaintextArchive {
    ...
    let layout = write_rem_tar_object_from_readers(&mut sink, &options, &mut files)
        .expect("base hardlink REM-OBJECT writes");
    let bytes = sink.blocks.into_iter().flatten().collect();
    PlaintextArchive {
        bytes,
        layout,
...
fn mutate_plaintext_archive(id: &str, archive: &mut PlaintextArchive) {
    match id {
        "wrong-format-id" => replace_once(&mut archive.bytes, b"rem-object-v1", b"rem-object-v0"),
...
) -> Result<Vec<u8>, RemObjectAeadError> {
    let metadata =
        RemObjectMetadata::new(options.plaintext_size, metadata_digest, options.chunk_size)?;
...
fn envelope_from_metadata_plaintext(
    ...
    let header = RemObjectHeader::new_envelope(
        ...
        u32::try_from(key_frame.len()).map_err(|_| RemObjectAeadError::SizeOverflow)?,
    )?;
    ...
    out.extend_from_slice(REM_OBJECT_FOOTER);
    ...
    let stored_size = usize::try_from(stored_size).map_err(|_| RemObjectAeadError::SizeOverflow)?;
...
fn non_derived_salt(options: &SealOptions) -> Result<[u8; 16], RemObjectAeadError> {
    let metadata = RemObjectMetadata::new(
        options.plaintext_size,
        options.plaintext_digest,
        options.chunk_size,
...
fn build_key_frame(plaintext: &[u8], options: &SealOptions) -> Result<Vec<u8>, RemObjectAeadError> {
...
fn assert_envelope_base(fixture: &Value) {
    ...
    assert_eq!(str_field(base, "recipient_mode"), "hpke-xwing-draft10");
...
fn replace_key_frame(object: &[u8], key_frame: &KeyFrame) -> Vec<u8> {
    ...
    let body_end = inspected.footer_offset as usize + REM_OBJECT_FOOTER.len();
...
fn one_slot_readable_envelope() -> (Vec<u8>, RecipientPrivateKey) {
    ...
    sealed.extend_from_slice(REM_OBJECT_FOOTER);
    let stored_size = stored_size_from_parts(
        header.chunk_size,
        header.key_frame_len,
        header.metadata_frame_len,
...
fn run_key_frame_case(id: &str, operation: &str) -> Result<(), RemObjectAeadError> {
    if matches!(
        id,
    ...
    match id {
        ...
        "oversize-key-frame" => {
            object[0x3c..0x40]
                .copy_from_slice(&((REM_OBJECT_KEY_FRAME_MAX_LEN + 1) as u32).to_be_bytes());
        }
        ...
        "hpke-undersized-key-frame-len" => {
            object[0x3c..0x40]
                .copy_from_slice(&((REM_OBJECT_KEY_FRAME_MIN_LEN - 1) as u32).to_be_bytes());
        }
...
fn plaintext_writer_negative_vectors_match_manifest_errors() {
    let fixture = fixture(include_str!(
        "../../../fixtures/rem-object/negative-writer.json"
    ));
...
fn plaintext_reader_negative_vectors_match_manifest_errors() {
    let fixture = fixture(include_str!(
        "../../../fixtures/rem-object/negative-plaintext-reader.json"
    ));
    assert_complete_case_ids(
        &fixture,
        &[
...
fn encrypted_inner_negative_vectors_match_manifest_errors() {
    let fixture = fixture(include_str!(
        "../../../fixtures/rem-object/negative-inner.json"
    ));
    assert_complete_case_ids(
        &fixture,
        &[
...
fn envelope_negative_vectors_match_manifest_errors() {
    let fixture = fixture(include_str!(
        "../../../fixtures/rem-object/negative-envelope.json"
    ));
    assert_complete_case_ids(
        &fixture,
        &[
...
fn key_frame_negative_vectors_match_manifest_errors() {
    let fixture = fixture(include_str!(
        "../../../fixtures/rem-object/negative-key-frame.json"
    ));
    assert_complete_case_ids(
        &fixture,
        &[
