...
use remanence_aead::{
    open_inner_range_to_vec, open_plaintext_range_to_vec, RangeOpenReport, RecipientPrivateKey,
};
...
pub fn read_encrypted_rao_file_range_to_vec(
    ...
    if range_len == 0 {
        let (bytes, envelope) = if let Some(first_chunk_lba) = first_chunk_lba {
            open_inner_range_to_vec(encrypted, recipient, first_chunk_lba.0, range_start, 0)?
        } else {
...
mod tests {
    ...
    fn encrypted_file_range_does_not_authenticate_unrequested_file_chunks() {
        ...
        let unrequested_chunk_offset =
            cipher_offset(key_frame_len, metadata_len, 512, first_chunk_lba.0 + 2).unwrap()
                as usize;
    ...
    fn encrypted_file_range_fails_closed_for_requested_chunk_damage() {
        ...
        let requested_chunk_offset =
            cipher_offset(key_frame_len, metadata_len, 512, first_chunk_lba.0 + 1).unwrap()
                as usize;
    ...
    fn empty_encrypted_file_range_authenticates_metadata_without_first_lba() {
        let (encrypted, _payload, recipient, _first_chunk_lba, _chunk_size, _, _metadata_len) =
            encrypted_object();
