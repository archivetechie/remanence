//! Verification extraction of the REM-OBJECT metadata deterministic-CBOR core.
//!
...
pub enum RemObjectMetadataError {
    InvalidChunkSize,
...
pub fn checked_add(a: u64, b: u64) -> Result<u64, RemObjectMetadataError> {
    match a.checked_add(b) {
        Some(sum) => Ok(sum),
        None => Err(RemObjectMetadataError::InvalidMetadataField),
    }
}

pub fn checked_mul(a: u64, b: u64) -> Result<u64, RemObjectMetadataError> {
...
pub fn validate_chunk_size(chunk_size: u64) -> Result<(), RemObjectMetadataError> {
    if chunk_size == 0 || chunk_size % CHUNK_SIZE_GRANULARITY != 0 {
        return Err(RemObjectMetadataError::InvalidChunkSize);
    }
    Ok(())
}
...
pub fn validate_metadata_core(
    ...
    chunk_size: u64,
) -> Result<(), RemObjectMetadataError> {
    validate_chunk_size(chunk_size)?;
    if metadata.plaintext_size == 0 || metadata.plaintext_size % chunk_size != 0 {
        return Err(RemObjectMetadataError::InvalidMetadataField);
    }
...
pub fn encode_metadata_core(
    metadata: MetadataCore,
    chunk_size: u64,
) -> Result<MetadataCborCore, RemObjectMetadataError> {
...
pub fn decode_metadata_core(
    ...
    chunk_size: u64,
) -> Result<MetadataCore, RemObjectMetadataError> {
    if wire.trailing_data {
        return Err(RemObjectMetadataError::InvalidCborEncoding);
    }
    if wire.map_len != REQUIRED_MAP_LEN {
        return Err(RemObjectMetadataError::MissingRequiredMetadataField);
    }
...
mod tests {
    ...
    fn drift_guard() {
        ...
        .expect("production metadata.rs must be readable from verif/rem-object-metadata");
        ...
        for (i, snippet) in extraction_snippets.iter().enumerate() {
            assert!(
                ...
                "extraction snippet {i} missing from verif REM-OBJECT metadata model"
            );
        }
    }
    ...
    fn validation_rejects_bad_plaintext_size() {
        ...
        assert_eq!(
            ...
            RemObjectMetadataError::InvalidMetadataField
        );
    ...
    fn decode_rejects_bad_writer_shape() {
        ...
        assert_eq!(
            ...
            RemObjectMetadataError::InvalidCborEncoding
        );
        ...
        assert_eq!(
            decode_metadata_core(bad, 262_144).unwrap_err(),
            RemObjectMetadataError::MissingRequiredMetadataField
        );
