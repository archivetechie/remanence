...
| `parity-capacity` | `crates/remanence-parity/src/{capacity,sidecar,parity_map,tape_index,replicated_control}.rs` | Existing reserve behavior plus the checked shared sidecar-layout result, ParityMap bound arithmetic, fixed-slot replicated snapshot sizing, structural-count and worst-close `C-H` profile feasibility, post-closeout projection, conservative close-bound composition, checked overflow, and capacity/spool gate order. | `ciborium` internals and the reviewed CBOR-bound derivation, live spool/tape IO, trusted bootstrap inline-fit refinement, geometric policy-bundle projection, and writer wiring while the Stage 0 kernel remains unused. |
...
| `pool-selection` | `crates/remanence-api/src/pool_selection.rs` | Fit/completion predicates, leftover arithmetic, ranking/tie-break order, and candidate-specific low/high/close-bound admission with checked retry behavior. | Iterator internals, catalog projection, drive occupancy projection, physical projection sourcing, and retry orchestration while the candidate helper remains unused. |
