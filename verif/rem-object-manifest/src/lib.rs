//! Verification extraction of REM-OBJECT manifest writer-schema cores.
//!
...
pub enum RemObjectManifestError {
    InvalidChunkSize,
...
pub fn checked_add(a: u64, b: u64) -> Result<u64, RemObjectManifestError> {
    match a.checked_add(b) {
        Some(sum) => Ok(sum),
        None => Err(RemObjectManifestError::InvalidManifestField),
    }
}

pub fn validate_chunk_size(chunk_size: u64) -> Result<(), RemObjectManifestError> {
    if chunk_size == 0 || chunk_size % CHUNK_SIZE_GRANULARITY != 0 {
        return Err(RemObjectManifestError::InvalidChunkSize);
    }
    Ok(())
}

pub fn chunk_count_core(size_bytes: u64, chunk_size: u64) -> Result<u64, RemObjectManifestError> {
...
pub fn validate_regular_file_core(
    ...
    chunk_size: u64,
) -> Result<(), RemObjectManifestError> {
    validate_chunk_size(chunk_size)?;
    if file.path_id == 0 || file.file_id == 0 {
        return Err(RemObjectManifestError::InvalidManifestField);
    }
...
pub fn validate_manifest_core(manifest: ManifestCore) -> Result<(), RemObjectManifestError> {
...
pub fn encode_regular_file_core(
    file: RegularFileCore,
    chunk_size: u64,
) -> Result<RegularFileWireCore, RemObjectManifestError> {
...
pub fn encode_manifest_core(manifest: ManifestCore) -> Result<ManifestWireCore, RemObjectManifestError> {
...
pub fn decode_regular_file_core(
    ...
    chunk_size: u64,
) -> Result<RegularFileCore, RemObjectManifestError> {
    if wire.map_len != FILE_ENTRY_REGULAR_MAP_LEN {
        return Err(RemObjectManifestError::MissingRequiredManifestField);
    }
    ...
    {
        return Err(RemObjectManifestError::InvalidCborEncoding);
    }
...
) -> Result<ManifestCore, RemObjectManifestError> {
...
pub fn validate_hardlink_entry_core(entry: HardlinkEntryCore) -> Result<(), RemObjectManifestError> {
...
pub fn validate_symlink_entry_core(entry: SymlinkEntryCore) -> Result<(), RemObjectManifestError> {
...
pub fn validate_directory_entry_core(entry: DirectoryEntryCore) -> Result<(), RemObjectManifestError> {
...
pub fn encode_rich_regular_file_core(
    file: RichRegularFileCore,
    chunk_size: u64,
) -> Result<RichRegularFileWireCore, RemObjectManifestError> {
...
) -> Result<LinkEntryWireCore, RemObjectManifestError> {
...
) -> Result<DirectoryEntryWireCore, RemObjectManifestError> {
...
pub fn encode_manifest_entries_core(
    manifest: ManifestEntriesCore,
) -> Result<ManifestEntriesWireCore, RemObjectManifestError> {
...
pub fn decode_rich_regular_file_core(
    ...
    chunk_size: u64,
) -> Result<RichRegularFileCore, RemObjectManifestError> {
    if wire.map_len != FILE_ENTRY_REGULAR_MAP_LEN {
        return Err(RemObjectManifestError::MissingRequiredManifestField);
    }
    if wire.key_path != FILE_KEY_PATH
...
) -> Result<HardlinkEntryCore, RemObjectManifestError> {
...
pub fn decode_symlink_entry_core(
    wire: LinkEntryWireCore,
) -> Result<SymlinkEntryCore, RemObjectManifestError> {
    if wire.map_len != FILE_ENTRY_LINK_MAP_LEN {
        return Err(RemObjectManifestError::MissingRequiredManifestField);
    }
    if wire.key_path != LINK_KEY_PATH
...
) -> Result<DirectoryEntryCore, RemObjectManifestError> {
...
pub fn decode_manifest_entries_core(
    ...
    reader_chunk_size: u64,
) -> Result<ManifestEntriesCore, RemObjectManifestError> {
    if wire.trailing_data {
        return Err(RemObjectManifestError::InvalidCborEncoding);
    }
    if wire.root_map_len != ROOT_MAP_LEN {
...
pub fn hardlink_target_seen_regular_prefix_two_core(
    ...
    Err(RemObjectManifestError::InvalidManifestField)
}

pub fn validate_manifest_array_core(manifest: ManifestEntriesCore) -> Result<(), RemObjectManifestError> {
    validate_chunk_size(manifest.chunk_size)?;
    if manifest.object_id == 0 {
        return Err(RemObjectManifestError::InvalidManifestField);
    }
...
pub fn validate_planner_entry_core(entry: PlannerEntryCore) -> Result<(), RemObjectManifestError> {
...
) -> Result<PlannerFoldStateCore, RemObjectManifestError> {
...
pub fn planner_entry_from_membership_core(
    source: PlannerSourceEntryCore,
    facts: PlannerMembershipFactsCore,
) -> Result<PlannerEntryCore, RemObjectManifestError> {
...
mod tests {
    ...
    fn drift_guard() {
        ...
        .expect("production layout.rs must be readable from verif/rem-object-manifest");
        let manifest = std::fs::read_to_string(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../crates/remanence-format/src/manifest.rs"
        ))
        .expect("production manifest.rs must be readable from verif/rem-object-manifest");
        ...
        for (i, snippet) in extraction_snippets.iter().enumerate() {
            assert!(
                ...
                "extraction snippet {i} missing from verif REM-OBJECT manifest model"
            );
        }
    }
