...
| --- | --- | --- | --- |
| REM-PARITY | 1.0.0-draft.5 (replacement implemented; pre-freeze fuzz and physical validation open) | 1.0.0-draft.2 | Terminal triple index: three complete final replicas, two typed separation extents, no intermediate indexes, exact close reserve, irreversible finalization/recovery authority, and manual early finalization. The terminal byte tables are part of the specification (Sections 8.3 and 10). Review-only minimal and multi-object vectors at every legal block size are under `../../fixtures/rem-parity-terminal-index-draft/`. Older bootstrap/index prose is superseded and is not compatibility authority. Independent derivation, proof gates, clean-slate VTL lifecycle, and the production-default one-GiB separation layout at all three legal block sizes have passed. Dedicated terminal-replica/separation/parser-walk fuzz plateaus and supervised physical-media validation remain open. Section 1.5 states which rules belong in the specification, and the rules it places outside the specification have left it. Most went to the REM Implementation and Operations Guide, whose chapters on writing, finalizing and reading tapes and on capacity admission this change writes. The description of the reference implementation's journals went to that implementation's documentation. The freeze criteria of Section 18, under their existing numbers, the candidate-vector rule and the negative-vector coverage list went to [“How a revision is frozen”](../README.md#how-a-revision-is-frozen). Section 3.4 now says that a Writer does not report a tape as `Finalized` while fewer than three complete replicas exist. Section 8.2 lists bootstrap key 3 as optional, and readers still tolerate its absence. No byte, valid tape or vector changed. |
