...
> point; an implementation is validated against it, not the reverse. The
> cryptographic values in the Section 13 test vectors that cannot be derived
> by arithmetic alone are marked *pinned-at-generation*; producing and
> freezing them against an independent re-derivation is a freeze criterion
> (Section 14). Items still open before freeze are collected in Appendix C.
...
in-memory interface (in the reference implementation, a 32-byte key file named
by `--key-file`, with the `key_id` supplied alongside). Implementations MUST
...
vectors — the expected Section 11 error name. Vectors use small `chunk_size`
...
1. A reference implementation implements this document — both representations,
   the plaintext stream and the encryption envelope.
...
9. A live round-trip passes on the QuadStor VTL and on the MSL3040 for both
   representations at two distinct `chunk_size` values, including
...
- [POSIX-PAX] — IEEE Std 1003.1, `pax` Interchange Format (ustar and pax
  extended headers).
...
1. **Pinned-at-generation vector values** (Section 13). The cryptographic
   ...
   Appendix A, and frozen into `fixtures/rao/`. They MUST additionally be
...
3. **Tape block size = `chunk_size`** (Section 8.2). A copy must be written to
   ...
   configuration.
4. **Verifier/salvage profile.** The reference implementation must expose the
